...
2. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   - 禁止指令重排序
     ...
     **你在那些地方用到过 volatile？**
     ...
     上面版本在单线程下完全没问题，但在多线程情况下会创建两个对象，也就是两个线程同时判断 `instance == null` 时都判断成功，往下走就同时 new 出了两个对象。需要优化为 <font color='red'>DCL 加锁版本</font>（Double-checked locking <span id='DCL'>双重校验锁</span> / 双端校验锁）：

     what：DCL 是给程序加 synchronized 代码块锁，同时在加锁前后都做一次 null 值判断，确认完全为 null 了才创建实例对象
