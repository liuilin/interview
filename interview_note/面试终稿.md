...
22. ==**synchronized 和 ReentrantLock（可重入锁）的区别**==

    同：两者都是可重入锁

    异：synchronized 是 java 里的关键字，而 ReentrantLock 是类。既然 ReentrantLock 是类，那么它就提供了比 synchronized 更多更灵活的特性，可以被继承、可以有方法、可以有各种各样的类变量

    synchronized 容易导致线程积压，它要等第一个线程释放了之后才执行下一个。比如一段代码里面有个方法要执行 5s，那么其它线程只有等它执行完

    ReentrantLock 可以在 try lock() 中给个规定时间，规定时间内拿不到锁才放弃
...
5. <span id='session'>**session共享怎么做的（分布式如何实现 session 共享）**</span>
...
2. **谈谈分布式锁（分布式锁如何设计）**
   ...
   > 分布式架构下，不同服务器的锁管不到其它服务器的锁，所以就得用分布式锁，项目中的落地方案采用的是 Redis

   分布式锁：控制在分布式架构里面的多个模块（多进程）访问资源的一个优先级

   > Synchronized 和 ReentrantLock 只能解决在单一进程（单机系统）之内的并发问题，不能解决分布式的情况
   ...
   > 问题：1. 删除失败怎么办	2. 这种 insert 语句不是一个重入锁，改造方法：每个节点搞个 id，insert 失败的时候去看这条记录的 id 是否等于当前进程的 id，如果是的话就证明是重入操作就可以继续做
...
1. **在项目中缓存是如何使用的？缓存如果使用不当会造成什么后果？**
   ...
   课程首页访问量非常大，我们就会让它走缓存，之前走数据库需要600ms的，现在直接走缓存2ms搞定了，性能提升300倍，保证了系统的高性能。特别是高 QPS 的系统，每次都去查询数据库的话，对于数据库来说将是个灾难。但是用上缓存就会出现各种问题
