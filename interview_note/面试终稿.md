...
2. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   - 保证可见性：当多个线程操作同一个共享变量时，就有可能触发线程并发问题。使用 volatile 关键字修饰的变量，保证了其在多线程之间的可见性，即每次读取到 volatile 变量，一定是最新的数据
   ...
   - 但它不保证原子性：加 Synchronized 可以解决，但是这个锁太重了，有点杀鸡用牛刀了。那么我们还可以用 [java.util.concurrent.atomic (Java Platform SE 8 )](https://docs.oracle.com/javase/8/docs/api/) 包下的 AtomicInteger来保证了原子性（底层是用的 CAS）

     > count++ 是不可分割的，只要读到了 V==E 就要把最新值赋给 V

     num++ 对应的 JVM 字节码（汇编字节码）解读，num++ 做了三步：
...
8. **volatile（可见性）和 Synchronized（锁）的区别**

   ![1554218515504](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/1554218515504.png)

   :one: JMM（Java 内存模型：Java Memory Model）设计的主要原理就是解决并发过程中如何处理可见性、原子性、有序性的问题
