...
1. **JVM的内存结构（模型）**
   ...
   - 栈：每一个方法就是一个栈帧（1.局部变量表 2.操作数栈 3.动态链接 4.返回地址）

     - 栈是什么：它的生命周期是跟随线程的，线程结束时栈内存也就释放了，所以对于栈来说不存在垃圾回收问题

     - 作用

       存储方法的局部变量，包括 8 种基本数据类型和对象的应用地址 reference address（真正的对象在堆空间中），并参与方法的调用与返回（也就是压栈和出栈）
     ...
     - java.lang.StackOverflowError
       ...
       递归调用后，在 Java 中函数栈调用层级过多，导致线程的栈满了，就会出现此异常。
   ...
   - 本地方法栈：执行非 Java 所写的代码（比如 io 最底层操作的汇编 Native 方法）
...
4. **谈谈你对类加载器的理解**
   ...
   **JVM类加载的过程（生命周期）| 类加载机制**
   ...
   - 初始化 Initialization：
     ...
     -  `<clinit>()` （class init）：不同于<font color='red'>类的构造器</font> ，构造器在 JVM 视角下对应的是 `<init>()` 方法，任何一个类声明以后，内部至少存在一个类的无参构造，所以任何一个类的字节码都会有 `<init>()` 
...
6. **JVM 垃圾回收**
   ...
   - 引用计数：里面有个计数器，有对象引用 +1，没对象引用 -1，但是有个天生的缺陷是较难处理循环引用问题，JVM 的实现一般不使用这种
   - <span id='fuzhiqingchu'>复制清除算法</span>：发生在年轻代，

   [JVM Garbage Collectors | Baeldung](https://www.baeldung.com/jvm-garbage-collectors)

7. **JVM 如何判断一个对象是否该被 GC，可以视为 root 的都有哪几种类型 **

8. **Java 中如何判断一个对象是否是一个垃圾 **
...
11. **Java中的四种引用类型及其含义（强软弱虚引用的区别以及GC对他们执行怎样的操作）**
...
13. **父类双亲委派机制**
    ...
    3. Application ClassLoader 应用类加载器：加载自己写的类
...
2. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   - 禁止指令重排序
     ...
     **你在那些地方用到过 volatile？**
     ...
     优化上面为 <font color='red'>DCL 版本</font>（Double-checked locking <span id='DCL'>双重校验锁</span> / 双端校验锁）：
...
- 使用线程池 Executors.newCachedThreadPool ()
