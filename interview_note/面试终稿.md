...
2. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   - 不保证原子性：
     ...
     what：不可分割，具有完整性，也即某个线程正在做某个具体业务时，中间不可以被加塞或分割。需要整体完整。要么同时成功，要么同时失败。

     加 Synchronized 可以解决，但是这个锁太重了，有点杀鸡用牛刀了。那么我们还可以用 [java.util.concurrent.atomic (Java Platform SE 8 )](https://docs.oracle.com/javase/8/docs/api/) 包下的 AtomicInteger来保证了原子性（底层是用的 CAS）

     

     num++ 是不可分割的操作，对应的 JVM 字节码（汇编字节码）解读，num++ 做了三步：
