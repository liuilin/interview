...
5. **Java 容器有哪些？哪些是同步容器？哪些是并发容器？（Java 集合之间的区别 | 介绍一下集合框架）**
   ...
   - Collection
     ...
     - ArrayList 和 Vector
       ...
       Vector 加了锁是线程安全的，数据一致性保证了，但是性能急剧下级。ArrayList 不是同步的，所以在不需要保证线程安全时建议使用 ArrayList
...
16. **一个ArrayList在循环过程中删除，会不会出问题，为什么**
...
[40个Java多线程问题总结 - 五月的仓颉 - 博客园](https://www.cnblogs.com/xrq730/p/5060921.html#4217608)
...
1. ~~**为什么要使用多线程（你了解的多线程）**~~
   ...
   >
   > 但是当多个线程操作同一个共享变量时，就有可能触发线程并发问题。那我们可以加锁，加锁呢有两种解决方案，一个是悲观锁，一个是乐观锁
...
2. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   volatile 是 JVM 提供的比 Synchronized 轻量级的同步机制，它有三大特性：

   - 保证可见性：当多个线程操作同一个共享变量时，就有可能触发线程并发问题。使用 volatile 关键字修饰的变量，保证了其在多线程之间的可见性，即每次读取到 volatile 变量，一定是最新的数据（保证所有线程都能看到内存中的最新数据）

     首先聊到可见性呢需要了解到 <font color='red'>JMM（Java 内存模型：Java Memory Model）</font>它主要是解决并发过程中的可见性、原子性和有序性问题
     ...
     ![JMM 可见性](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/JMM%20%E5%8F%AF%E8%A7%81%E6%80%A7.jpg)

     > 了解原理即可：
     >
     > JMM（Java 内存模型 Java Memory Model, 简称 JMM）本身是一种抽象的概念并不真实存在，它描述的是一组规则或规范通过规范定制了程序中各个变量（包括实例字段，静态字段和构成数组对象的元素）的访问方式
     >
     > JMM 关于同步规定：
     >
     ...
     > 3. 加锁解锁是同一把锁
     >
     > 由于 JVM 运行程序的实体是线程，而每个线程创建时 JVM 都会为其创建一个工作内存（有些地方成为栈空间）, 工作内存是每个线程的私有数据区域，而 Java 内存模型中规定<font color='red'>所有变量都存储在主内存，主内存是共享内存区域，所有线程都可访问，但线程对变量的操作（读取赋值等）必须在工作内存中进行，首先要将变量从主内存拷贝到自己的工作空间，然后对变量进行操作，操作完成再将变量写回主内存</font>，不能直接操作主内存中的变量，各个线程中的工作内存储存着主内存中的变量副本拷贝，因此不同的线程无法访问对方的工作内存，线程之间的通讯（传值）必须通过主内存来完成
   ...
   - 禁止指令重排序
     ...
     比如高考试卷发下来一般是先把会做的做了，不按照题的顺序做。但是用了 volatile 禁止指令重排之后，相当于强制要求必须按题的顺序做，不可以跳着做

     > 计算机在执行程序时，为了提高性能，编译器和处理器一般会做<font color='red'>指令重排</font>，那分为三点：
     >
     > i. 单线程环境下要确保程序最终执行结果和代码顺序执行的结果一致
     >
     > ii. 处理器在进行重排序时必须要考虑指令之间的<font color='red'>数据依赖性</font>（比如 x = x + 5，要先有 int x = 0 才行）
     >
     > iii. 多线程环境下线程是交替执行的，由于编译器优化重排的存在，两个线程使用的变量能否保持一致性就无法确定，以至于结果也无法预测
     >
     > ![image-20210729181549413](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/image-20210729181549413.png)
     ...
     // 1324
     // 问题：请问可不可以重排序为 4321
     // 不可以，重新排序要考虑指令之间的数据依赖性，要先有 x，才能计算 y = x * x（不可能先有的我，再有的我妈）
     ```
     ...
     | 指令重排前 | x = a;        | y = b;                                     |
     |            | b = 1;        | a = 2;                                     |
     |            | x = 0; y = 0; | 不是每一次都会指令重排，正常指令不重排情况 |
     | 指令重排后 | b = 1;        | a = 2;                                     |
     |            | x = a;        | y = b;                                     |
     |            | x = 2; y = 1; | 多线程导致指令重排，指令重排后情况         |
     ...
     > <font color='red'>上面例子有错误，待考证</font>
     >
     > 情况一：正常指令不重排情况，两个线程先 `语句 1` 执行到 `语句 2` ，此时线程 2 被挂起且 a = 1; 线程 1 接着走完语句 3，这时 a = 6; 线程 2 被唤醒执行到语句 3，此时 a = 6 + 5 = 11; 最终打印 11
     >
     > 情况二：不重排直接执行完，a = 6;

     ==线程安全性如何获得保障**总结：**工作内存与主内存同步延迟现象导致的可见性问题。可以使用  synchronized（只允许一个线程来修改） 或 volatile 关键字解决，它们都可以使一个线程修改后的变量立即对其他线程可见==

     ==对于指令重排导致的可见性问题和有序性问题，可以利用 volatile关键字解决，因为 volatile的另外一个作用就是禁止重排序优化==
     ...
     **你在那些地方用到过 volatile？**
     ...
     i. 在内存里只有一个实例，减少了内存开销。特别是这个对象需要频繁的创建而且创建的过程又无法优化

     ii. 避免对资源的多重占用

     iii. 设置全局访问点，严格的控制访问。也就是说对外不让 new 出来，只能通过我这个方法来创建单例对象

     缺点：没有接口，扩展的话就比较困难。每次都得改代码
     ...
     优化上面为 <font color='red'>DCL 版本</font>（Double-checked locking 双重校验锁 / 双端校验锁）：

     what：在加锁前后都做一次判断，确认完全没问题了才创建实例对象

     why：把 synchronized 放到静态代码块上可以在多线程情况下保证单例，但是并不推荐这样

     - 因为它太重了：锁住整个方法（如果修饰 static 方法时是锁住的整个类）。虽然保证了数据一致性，但是并发性和系统性能就下降了
       ...
       懒汉式注重延时加载，要使用它的时候才开始初始化

       > DCL 机制不一定线程安全，原因是有指令重排的存在，加入 volatile 可以禁止指令重排
       >

     - 上面程序运行成千上万次可能都没有问题，但是有可能上千万次有一次错误。因为底层有指令重排的存在（用 volatile 禁止指令重排），为什么使用 volatile 分析：

       > 原因在于某一个线程在执行到第一次检测读取到的 instance 不为 null 时，instance 的引用对象可能没有完成初始化
       >
       > instance = new Singleton(); 
       >
       > 可以分为以下步骤 (伪代码) 
       ...
       >
       > instance = memory;	// 3. 设置 instance 的指向刚分配的内存地址，此时才 instance != null
       >
       > 步骤 2 和步骤 3 不存在数据依赖性，而且无论指令重排前还是重排后程序执行的结果在单线程中并没有改变，因此这种重排优化是允许的，那么重排序之后步骤 2、3 顺序就可能发生变化，先执行步骤 3 来设置 instance 的指向刚分配的内存地址。接着另一个现在进来判断 instance 不为 null，就直接返回了未初始化对象的实例，此时就会报错，出现了线程安全问题
       >
       > memory = allocate ();	// 1. 分配对象内存空间
       >
       > instance = memory;	// 3. 设置 instance 的指向到刚分配的内存地址后，其它线程发现 instance 已经指向了内存空间，不为 null。就直接返回实例，线程 1 访问的是线程 0 还未初始化完成的对象，此时系统就会报异常
       >
       > instance (memory);	// 2. 初始化对象
       >
       > 但是指令重排只会保证串行语义的执行一致性 (单线程) 并不会关心多线程间的语义一致性
       >
       > 所以当一条线程访问 instance 不为 null 时，由于 instance 实例不一定初始化完成，也就造成了线程安全问题。
       >
       > ![image-20210730163454813](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/image-20210730163454813.png)

       singleton = new Singleton(); 这行代码并不是一个原子指令，可能会在JVM中进行指令重排；

       new 实例背后的指令，我们通过使用 javap -c 指令，查看字节码如下：
   ...
   - 不保证原子性：
     ...
     what：原子性是不可分割且保证完整性，也就是某个线程正在做某个具体业务时，中间不可以被加塞或分割。需要整体完成。要么同时成功，要么同时失败。

     由于 num++ 是不可分割的操作，对应的 JVM 字节码（汇编字节码）解读出来 num++ 做了三个步骤：

     i. getfield：获得初始值	// Field n:I（int）

     ii. iconst_1 + iadd：执行 +1

     iii. putfield：把累加后的值写回主内存
     ...
     在操作 num++ 时，首先线程会把主内存对象数据拷贝到各自线程的工作内存，T1 线程进行 +1 操作然后写回主内存，正要通知其他线程的时候被挂起了。此时 T2 线程也进行 num++ 操作把数据 11 写回主内存了，这时之前的 num = 11 就被覆盖。因此本来三个线程应该累加到 13 的，由于上面数据被覆盖了，导致无法完成原子操作，最终数据就变为 11。

     **如何解决原子性问题：**

     加 Synchronized 可以解决，但是这个锁太重了，有点杀鸡用牛刀的感觉。那除此外呢我们还可以用 [java.util.concurrent.atomic (Java Platform SE 8 )](https://docs.oracle.com/javase/8/docs/api/) 包下的 AtomicInteger 来保证原子性（底层是用的 CAS）
...
3. **悲观锁 VS 乐观锁，请讲一下它们的优缺点**

   悲观锁（Synchronized）与乐观锁（CAS）是一种广义上的概念，体现了看待线程同步的不同角度。在 Java 和数据库中都有此概念对应的实际应用。

   what：

   - 先说概念。对于同一个数据的并发操作，悲观锁认为自己在使用数据的时候一定有别的线程来修改数据，因此在获取数据的时候会先加锁，确保数据不会被其它的线程修改。Java 中 synchronized 关键字和 Lock 的实现类都是悲观锁。

   - 而乐观锁认为自己在使用数据时不会有别的线程修改数据，所以不会添加锁，只是在更新数据的时候去判断之前有没有别的线程更新了这个数据。如果这个数据没有被更新，当前线程会将自己修改的数据写入主内存。如果数据已经被其他线程修改，则根据不同的实现方式执行不同的操作（例如报错或者自旋来自动重试）。

     > 乐观锁在 Java 中是通过使用无锁编程来实现，最常采用的是 CAS 算法，Java 原子类中的递增操作就通过 CAS 自旋实现的。

   how：

   - 悲观锁适合写操作多的场景，先加锁可以在写操作时保证数据一致性。

     > 传统的关系型数据库里边就用到了很多这种锁机制，比如行锁，表锁等，读锁，写锁等，都是在做操作之前先上锁
   - 乐观锁适合读操作多的场景，不加锁的特点能够使其读操作的性能大幅提升。增大系统吞吐量
...
4. **讲一讲 Atomiclnteger，为什么要用 CAS 而不是 synchronized？（CAS知道吗？如何实现的 | CAS有什么缺陷，该如何解决）**

   CAS ---> Unsafe ---> CAS 底层思想---> ABA问题 ---> 原子引用更新 ---> 如何规避 ABA 问题
   ...
   CAS（全称为：Compare And Swap 比较并交换）：是一种无锁算法，在不使用锁（没有线程被阻塞）的情况下实现多线程之间的变量同步。底层是 <font color='red'> 一条 CPU 并发原语 </font> ，功能是把物理内存的真实值和线程的期望值进行比较，如果相同就把最新值赋给主物理内存值，否则就会重新获得主物理内存的最新值，循环操作。并发原语的指令执行是原子的，执行过程连续且不可被中断。也就不会造成数据不一致的问题，说明它是线程安全的
   ...
   **CAS 底层原理是什么？如果知道，谈谈你对 Unsafe 的理解**
   ...
   > 查看 AtomicInteger 的自增函数 getAndIncrement () 的源码时，发现自增函数底层调用的是 unsafe.getAndAddInt ()。但是由于 JDK 本身只有 Unsafe.class，只通过 class 文件中的参数名，并不能很好的了解方法的作用，所以我们通过 OpenJDK 8 来查看 Unsafe 的源码：
   ...
   public final int getAndAddInt(Object o, long offset, int delta) {
      int v;
      do {
          // 期望值：获得当前对象的内存地址的值（把主物理内存值拷贝到工作内存）
   ...
   根据 OpenJDK 8 的源码我们可以看出，用 getIntVolatile () 获得当前对象的内存地址值（相当于把主物理内存值拷贝到本地线程的工作内存），然后判断如果 `内存值` 与工作内存的 `期望值 v` 相等，那么就替换主内存的值为 `v + 需要变动的值 delta（v + 1）`，否则返回 false，继续循环进行重试，直到设置成功才能退出循环，并且将旧值返回。最最底层就是 JDK 通过调用汇编指令 cmpxchg（compare and exchange）

   > 整个 “比较并更新” 操作封装在 compareAndSwapInt () 中，在 JNI（Java Native Interface） 里是借助于一个 CPU 指令完成的，属于原子操作，可以保证多个线程都能够看到同一个变量的修改值。

   然后

   1. Unsafe 类是 CAS 的核心类，由于 Java 方法无法直接访问底层系统，而需要通过本地（native）方法来访问。Unsafe 相当于开了一个后门，基于该类的内部方法可以像 C 指针一样直接操作内存的数据。

   2. 变量 valueOffset，表示该变量值在内存中的偏移地址，因为 Unsafe 是根据内存偏移地址来获取数据的

      > valueOffset（内存地址偏移量）：不知道某同学叫什么，但是可以直接喊四排三列的同学
   ...
   **为什么要用 CAS 而不是 synchronized？**
   ...
   synchronized 加锁之后，同一时间段只能有一个线程来访问，一致性得到了保障，但并发性下降。而 CAS 是无锁，不断的比较并替换，这样就既保证了数据一致性，又保证了并发性

   **CAS缺点：**

   1. **循环时间长开销大：**CAS 操作不成功会导致一直自旋，里面有个 do while 循环，会给 CPU 带来非常大的开销。
   ...
   3. **ABA 问题：**首先是先把数据拷贝到 T1、T2 线程的工作内存，CAS 在操作值的时候会去检查内存值是否发生变化，没有发生变化才会更新内存值。比如内存值原来是 A，后来变为了 B，接着又变为了 A，然后 T1 线程挂起，T2 线程进行 CAS 比较时发现内存值没有发生变化，然后改为最新值。但是实际上内存值是有变化的，这就是 CAS 的 ABA 问题。
      ...
      - JDK 从 1.5 开始提供了 AtomicStampedReference 类增加时间戳的方式来解决 ABA 问题
...
5. **谈谈你对 Synchronized 关键字的理解（Synchronized关键字的用法，优缺点）**
   ...
   早期， jdk 早期的时候，这个 synchronized 的底层实现是重量级锁，重量级到这个 synchronized 要去找操作系统去申请锁的地步，这就会造成效率非常低。后来 Java 开始越来越多的处理高并发程序，大家不太满意，说这个 synchrionized 太重了，不好用，后来改进之后才有了锁升级的概念

   这个锁升级的概念呢，是这样的，原来呢都要去找操作系统，要找内核去申请这把锁，到后期做了对 synchronized 的一些改进，他的效率比原来要改变了不少，改进的地方。当我们使用 synchronized 的时候 HotSpot 的实现是这样的：上来之后第一个去访问某把锁的线程 比如 sync (Object) ，来了之后先在这个 Object 的头上面 markword 记录这个线程。（如果只有第一个线程访问的时候实际上是没有给这个 Object 加锁的，在内部实现的时候，只是记录这个线程的 ID（偏向锁））
...
8. **Java 实现多线程有哪几种方式。**
   ...
   > 正常情况下是两种，如果问到是哪三种的话？
   >
   > 回答加上 java 8 的 lamda 表达式，还是不认的话就说线程池方式 Executors.newCachedThread

   请你告诉我启动线程的三种方式？

   i. new Thread ().start ()； 

   ii. new Thread (Runnable).start ()

   > 这没问题那第三个呢？要回答线程池也是用的这两种之一，他这么问有些吹毛求疵的意思，你就可以说通过线程池也可以启动，一个新的线程

   iii. Executors.newCachedThreadPool () 或者 FutureTask + Callable
...
9. **我们知道 ArrayList 是线程不安全的，请编写一个不安全的案例并给出解决方案**
   ...
   ![CopyOnWrite 写时复制原理](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/CopyOnWrite%20%E5%86%99%E6%97%B6%E5%A4%8D%E5%88%B6%E5%8E%9F%E7%90%86.jpg)
...
11. **Lock接口有哪些实现类，使用场景是什么。**

12. **可重入锁的用处及实现原理，写时复制的过程，读写锁，分段锁（ConcurrentHashMap中的segment）**

13. **如何控制线程执行的顺序（ABC三个线程如何保证顺序执行）**
...
19. <span id='lockDiff'>**Synchronized 和 ReentrantLock（可重入锁）的区别**</span>
...
20. CopyOnWrite容器即写时复制的容器。往一个容器添加元素的时候，不直接往当前容器 Object[] 添加，而是先将当前容器 Object[] 进行copy，复制出一个新的容器 Object[] newELements，然后新的容器 Object[] newElements里添加元素，添加完元素之后， 再将原容器的引用指向新的容器setArray (newElements);。这样做的好处是可以为CopyOnWrite容器进行并发的读，而不需要加锁，因为当前容器不会添加任何元素。所以CopyOnWrite容器也是一种读写分离的思想，读和写不同的容器（我们用上一个，写完了用新的）
...
22. **线程池的参数有哪些，在线程池执行流程（创建一个线程的过程）**
