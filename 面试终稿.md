...
16. **双亲委派模型的过程以及优势**
    ![1553178799583](1553178799583.png)
...
![1553052790171](1553052790171.png)
...
6. **synchronized关键字的用法，优缺点**
   ...
   ![1553322829198](1553322829198.png)
...
7. **volatile（可见性）和synchronized（锁）的区别**
   ...
   ![1554218515504](1554218515504.png)
...
11. **如何控制线程执行的顺序（ABC三个线程如何保证顺序执行）**
    ...
    ![1554214612631](1554214612631.png)
...
11. **explain分析语句**
    ...
    - 执行explain包含的信息
      ...
      ![img](8dcf3edf-3423-4229-aef0-e9dbf1d42597-877132.jpg)
    ...
    - ==id字段三种情况介绍==（优化重要点）
      ...
      - 在id相同情况下
        ...
        ![img](ef75044f-246a-40c1-b2b7-faf323b6f34d-877132.jpg)

      - 在id不同的情况下

        子查询，id的序号会递增，id值越大优先级越高，越先被执行

        ![img](dc10b860-29e8-4d00-91b4-cb79360ad708-877132.jpg)
      ...
      - 在id相同和不同同时存在的情况下
        ...
        - DERIVED表示衍生，意思是s1是通过t3衍生得来的
          ...
          ![img](bdda6cdc-5b73-4e64-ae35-11a1c938ca77-877132.jpg)
...
12. **索引失效**
    ...
    ![img](cb4572c4-c85e-4eeb-9cbe-e04a56bb4b7c-877132.jpg)
    ...
    - 最佳左前缀法则
      ...
      - 依次执行三条查询SQL，索引情况正常，都使用到了
        ...
        ![img](c6e30510-90c1-4246-9f17-1fcd3ed51e6e-877132.jpg)

      - 出现索引失效情况

        未按照索引建立的顺序，跳过第一个索引name，直接使用后面两个查询

        ![img](0367427b-ea74-4008-933e-236bf9c234b1-877132.jpg)
    ...
    - 不要在索引列上做任何（计算、函数、（自动or手动）类型转换）操作，会导致索引失效而转向全表扫描
      ...
      ![img](4de861be-977e-43e2-99e9-8478dc2b433c-877132.jpg)

    - 存储引擎不能使用索引中范围条件右边的的列

      当索引条件中出现了 >、<、in一类的范围条件时，会导致索引失效

      ![img](1d342ffd-16cd-4768-b302-1997d3935f7d-877132.jpg)

    - 尽量使用覆盖索引，只访问索引查询，减少select *

      使用select * 和 select 字段，在extra字段上会有一定性能差距

      ![img](e7a1004e-515f-4b62-9ed6-0a58dd62a15b-877132.jpg)

    - mysql中使用is null 或者 is not null，会导致索引失效

      ![img](b32c77e7-c197-4602-ab9b-cc318d9879b0-877132.jpg)

    - like以通配符开关 '%adb'，会导致索引失效

      - 当使用like查询时，只 % 在最右边时，mysql才会使用到索引

        ![img](fda5e070-a0a2-4842-9015-37f20388454c-877132.jpg)

      - 如果必须要使用 like %abd%，的解决方案，使用覆盖索引，建立的索引和查询的字段在顺序，个数上最好保持一致

        ![img](7d26f956-61b3-41e2-ac6b-0e9c990ae14b-877132.jpg)

    - 字符串不加单引号索引失效

      当name查询条件值跟的是数字时，如果不加单引号，mysql会自动隐式转换为数字

      ![img](a2e7cac8-a6b5-4a8e-9774-b5819d4bee32-877132.jpg)
...
1. **在项目中缓存是如何使用的？缓存如果使用不当会造成什么后果？**
   ...
   ![01_缓存是如何实现高性能的](01_缓存是如何实现高性能的.png)
...
2. **Redis线程模型**
   ...
   ![01_Redis单线程模型](01_Redis单线程模型.png)
...
11. **如何处理Redis雪崩？**
    ...
    ![02_如何解决缓存雪崩](02_如何解决缓存雪崩.png)

    

12. **如何处理Redis缓存穿透？**
    ![03_缓存穿透现象以及解决方案](03_缓存穿透现象以及解决方案.png)
...
15. **高并发场景下的缓存+数据库双写不一致问题分析与解决方案设计**
    ...
    高并发场景：![复杂的数据库+缓存双写一致保障方案](复杂的数据库+缓存双写一致保障方案.png)
...
16. **Redis并发竞争问题以及解决方案？分布式锁？**
    ...
    ![01_Redis并发竞争问题以及解决方案](01_Redis并发竞争问题以及解决方案.png)
...
1. **SpringMVC处理执行流程（请求原理）**
   ...
   ![SpringMVC处理执行流程](10173293-005aa35c51e174dd.webp)
...
5. **解释Spring框架中bean的生命周期。**
   ...
   ![1553956304046](1553956304046.png)
...
1. **为什么使用消息队列啊**
   ...
   - 解耦：现在我们有这么一个场景，A系统调用BCD三个系统接口，然后发送数据，那如果E系统也要这个数据呢？那如果C系统现在不需要了呢？现在A系统又要发送第二种数据了呢？A系统负责人濒临崩溃中...再来点更加崩溃的事儿，最让A系统那哥们烦的是如果C系统如果挂了咋办？超时怎么办？要不要做重试机制？
     ![01_不用MQ系统耦合的场景](01_不用MQ系统耦合的场景.png)

     面对这么复杂的调用，维护起来超级的麻烦。这时我们采用MQ给他异步化解耦，就不需要直接调接口了

     ![02_使用了MQ之后的解耦场景](02_使用了MQ之后的解耦场景.png)
   ...
   ![03_不用MQ的同步高延时请求场景](03_不用MQ的同步高延时请求场景.png)

   ![04_使用MQ进行异步化之后的接口性能优化](04_使用MQ进行异步化之后的接口性能优化.png)

   - 削峰：每天0点到11点，A系统风平浪静，每秒并发请求数量就100个。结果每次一到11点~1点，每秒并发请求数量突然会暴增到1万条。但是系统最大的处理能力就只能是每秒钟处理1000个请求啊...尴尬了，系统会死
     ![05_没有用MQ的时候高峰期系统被打死场景](05_没有用MQ的时候高峰期系统被打死场景.png)
     ![06_使用MQ来进行消峰的场景](06_使用MQ来进行消峰的场景.png)
...
2. **消息队列有什么优点和缺点**
   ...
   ![07_架构中引入MQ后可能存在的一些问题](07_架构中引入MQ后可能存在的一些问题-1552656197216.png)
...
4. **如何保证消息队列的高可用**
   ...
   - RabbitMQ的高可用性
     ...
     - 普通集群模式
       ...
       ![01_RabbitMQ的普通集群模式原理分析](01_RabbitMQ的普通集群模式原理分析.png)
     ...
     - 镜像集群模式
       ![02_RabbitMQ的镜像集群模式](02_RabbitMQ的镜像集群模式.png)这种模式，才是所谓的RabbitMQ的高可用模式，跟普通集群模式不一样的是，你创建的queue，无论元数据还是queue里的消息都会存在于多个实例上，然后每次你写消息到queue时，都会自动同步消息到多个实例的queue。这样的话，好处在于你任何一个机器宕机了，别的机器都可以用。坏处在于：1.性能开销也太大了，消息同步所有机器，导致网络带宽压力和消耗很重 2.这个根本没有扩展性可言了，如果某个queue负载很重，你加机器，新增的机器也包含了这个queue的所有数据，并没有办法线性扩展你的queue
...
5. **如何保证消息不被重复消费啊（如何保证消息消费时的幂等性）**
   ...
   ![01_kafka消费端可能出现的重复消费问题](01_kafka消费端可能出现的重复消费问题.png)
...
6. **如何保证消息的可靠性传输（如何处理消息丢失的问题）**
   ...
   ![01_RabbitMQ可能存在的数据丢失问题](01_RabbitMQ可能存在的数据丢失问题.png)
...
7. **如何保证消息的顺序性**
   ...
   ![01_RabbitMQ可能存在的顺序错乱问题](01_RabbitMQ可能存在的顺序错乱问题.png)

   ![02_RabbitMQ如何保证消息的顺序性](02_RabbitMQ如何保证消息的顺序性.png)
...
1. **es的分布式架构原理能说一下么（es是如何实现分布式的）**
   ![01_elasticsearch分布式架构原理](01_elasticsearch分布式架构原理.png)
...
2. **ES写入数据的工作原理是什么啊**
   ...
   ![01_es读写底层原理剖析](01_es读写底层原理剖析.png)
...
2. **CMS页面管理模块**
   ...
   ![1552897686356](1552897686356.png)
...
7. **事务是怎么控制的？用到分布式事务控制了吗？如何做的？**
   ...
   ![1553048376228](1553048376228.png)
...
![01_高并发系统的架构组成](01_高并发系统的架构组成.png)
...
16. **学成在线的技术架构**
    ...
    ![1555770911092](1555770911092.png)
