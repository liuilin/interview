...
7. **Spring AOP 的底层实现原理？**
   ...
   what：面向切面编程 (Aspect-Oriented Programming)，只是 IOC 的一个扩展功能点 （一个BeanPostProcessor），它能够在不修改源码的情况下对其他功能进行增强。一般都是将系统中的一些非核心业务提取出来进行单独处理。比如事务、日志等等

   分：

   i. bean 的创建过程中有个步骤可以对 bean 进行扩展，AOP 本身就是一个扩展功能，所以在 BeanPostProcessor 的后置处理方法中来进行实现

   ii. 代理对象的创建过程，会先获取 advice、切面、切点的信息

   iii. 通过 JdkProxy 或 Cglib 的方式来生成对象

   iv. 在执行方法调用的时候，会调用到生成的字节码文件中，直接回找到 DynamicAdvisoredInterceptor 类中的 intercept 方法，从此方法开始执行

   v. 根据之前定义好的通知来生成拦截器链

   vi. 从拦截器链中依次获取每一个通知开始进行执行，在执行过程中，为了方便找到下一个通知是哪个，会有一个 CglibMethodInvocation 的对象
...
9. **你肯定知道 Spring，那说说 AOP 的全部通知顺序 SpringBoot1 或 SpringBoot2 对 AOP 的执行顺序影响？ | SpringBoot1 和 SpringBoot2 有什么区别**
