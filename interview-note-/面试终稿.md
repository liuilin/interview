...
> 临阵磨枪：线程池、AQS、分布式事务 Seata 原理、Spring IOC、Spring AOP、Bean 流程、Spring 循环依赖、MySQL（聚簇索引、非聚簇索引）、SQL 优化、Redis 原理、反射，jvm 调优（线上调优参数），NIO、IO 多路复用（Redis），高并发秒杀方案、CAP，MQ，Redis
>
> 线程池助记：Cartier makes Kate.Perry unbelievable，with two heads
>
> CAS 助记：CU底缺原，无多并内原，mm同高性，原使等加绑
>
> synchronized 助记：重 互 简 长 低 高 减。重要的人总是互相简单的比手长、比脂肪低、比身高、比减肥
>
> AQS 助记：lock try add queue。锁试图加入队列
>
> AOP 助记：b-代-J-字节码-通知-拦，bean代表JJ叫字节码通知拦截器链
>
> SQL 助记：null like exists 5 左，null 值喜欢存在5个左右
>
> JVM 优化助记（4 大垃圾回收算法，7 大垃圾回收器）：top, vmstat, free, iostat, ifstat。Top video filter iphone ios；出故障排查流程：top, ps ef, ps mp, jstack。top public policy journals 顶级政策期刊
>
> MQ 助记：高可用，重复消费，可靠性传输，顺序性，积压。高圆圆重新打开可乐顺利积攒瓶盖
>
> Redis 助记：雪崩：集群、本地缓存+降级、持久化；缓存穿透；缓存击穿；热点数据集中失效
>
> [所有临阵磨枪题都有【MySQL】mysql聚簇索引和非聚簇索引的区别_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV18S4y1r7NW?p=27&vd_source=4844de7cb051be29fbaf4555af0bbd8b)
>
> [聊聊 Linux 五种 IO 模型 - 简书](https://www.jianshu.com/p/486b0965c296)：计算机存在内核空间和用户空间这个东西
>
> [同步IO异步IO的本质区别是什么_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1JW4y187AL?p=24&vd_source=4844de7cb051be29fbaf4555af0bbd8b)
>
> 同步IO（阻塞、非阻塞、多路复用）：数据是先到内核的，是由程序自己从内核拷贝回来再处理
>
> 异步IO：程序不用单独去拷贝数据了
>
> MySQL（聚簇索引、非聚簇索引）：首先索引是存在磁盘的而不会在内存中，因为每次生成索引还是比较消耗性能。存储在磁盘的话就涉及到了 MySQL 的存储引擎，InnoDB 的数据文件和索引文件是全放在 idb 文件中的，而 MyISAM 的数据文件放在 myd 文件中，索引放在 myi 文件中，其实区分聚簇索引和非聚簇索引非常简单，只要判断数据跟索引是否存储在一起就可以了
>
> InnoDB 在进行数据插入的时候，数据必须要跟索引放在一起，如果有主键就使用主键，没有主键就使用唯一键，没有唯一键就使用 6 字节的 rowid，因此跟数据绑定在一起的就是聚簇索引。（但如果有多个索引，InnoDB）而为了避免数据冗余存储，其他的索引的叶子节点中存储的都是聚簇索引的 key 值，因此 InnoDB 中既有聚簇索引也有非聚簇索引，而 MyISAM 中只有非聚簇索。

JVM 调优：用的 undertow，压测后比 Tomcat 高出 3000 左右的 QPS，订单、物流等微服务 Xms、Xmx 设置的大一点 1024m，这两个参数调成一样大来避免 GC 频繁的收集。用的 G1 垃圾收集器来优化自动调节。元空间设置大一点改为 1024m，虽然在堆外但是默认只有 20m 左右，频繁 new 一些对象的话就会撑爆

JVM 调优参数：

- -Xms（ -XX:+InitialHeapSize）memory startup：初始化内存大小，默认为物理内存的 1/64
- -Xmx（-XX:+MaxHeapSize）memory max：最大内存大小，默认为物理内存的 1/4
- -Xss（-XX:+ThreadStackSize）stack size：单个线程栈大小，默认为 512k~1024k（值依赖于是哪个平台）
- -Xmn：新生代大小

思想层面：如果让你来设计一个 Spring 框架你会怎么做？如果让你来设计一个 Dubbo 框架你会怎么做？如果让你来设计一个 MyBatis 框架你会怎么做？如果让你来设计一个消息中间件你会怎么做？

> `/**` 的注释好像会导致本文档错乱，不要在 `> 引用` 中使用`1. 或 - `，不要搞多个空白换行格式会乱，目前暂时记录一下，`> 引用` 中换行偶尔也会导致错乱需要删除没对齐的引用符号
>
...
2. **重载和重写的区别**
   ...
   重载：发生在同一个类中，方法名必须相同，而参数类型、个数、顺序都不同，还有就是方法返回值及访问修饰符也可以不同。Spring 中的 `loadBeanDefinitions()` 里面就有很多重载方法
...
6. **Java 容器有哪些？哪些是普通容器？哪些是并发容器？（Java 集合之间的区别 | 介绍一下集合容器）**
   ...
   ![Java 集合容器](http://cdn.liumulin.top/interview/Java%20%E9%9B%86%E5%90%88%E5%AE%B9%E5%99%A8.jpg)
   ...
   - Collection
     ...
     - ArrayList 和 LinkedList
       ...
       **LinkedList 原理**
       ...
       ![链表](http://cdn.liumulin.top/interview/%E9%93%BE%E8%A1%A8.jpg)
     ...
     - 我们知道 ArrayList 是线程不安全的，请编写一个不安全的案例并给出解决方案（方案：CopyOnWriteArrayList）
       ...
       ![CopyOnWrite](http://cdn.liumulin.top/interview/CopyOnWrite.jpg)
...
8. **HashMap 底层数据结构？实现原理？源码？**
   ...
   - 1.8 做了什么优化？
     ...
     ![HashMap data structure jdk8](http://cdn.liumulin.top/interview/HashMap%20data%20structure%20jdk8.jpg)
...
9. **Hash 冲突**
   ...
   **扰动函数**
   ...
   ![HashMap 容量为 2 的 n 次幂](http://cdn.liumulin.top/interview/HashMap%20%E5%AE%B9%E9%87%8F%E4%B8%BA%202%20%E7%9A%84%20n%20%E6%AC%A1%E5%B9%82.jpg)
...
10. **HashMap 是线程安全的吗？不安全会导致哪些问题？如何解决？（HashMap 在高并发下如果没有处理线程安全会有怎样的安全隐患，具体表现是什么？）**
    ...
    ii. 不安全：每个线程想初始化容量大小可能不同，会出现线程安全问题。ConcurrentHashMap 是在 `initTable()` 中，把成员变量 `sizeCtl` 用 volatile 修饰来保证内存的可见性，同时用 `U.compareAndSwapInt(this, SIZECTL, sc, -1)` CAS 自旋来更改值，性能也大大提高

    iii. 不安全：进行数组判空检查时，会出现线程安全问题。ConcurrentHashMap 还是使用 `U.getObjectVolatile()` 先获取内存中的最新值，然后在进行判断。判断通过后用 CAS 来替换插入值

    iv. 不安全：还是在进行数组判空检查时，会出现线程安全问题。而且里面有分了复杂的替换、红黑树、链表三种情况，所以 ConcurrentHashMap 采用了分段锁方案，用 synchronized(f) 代码块来锁住当前数组下标那条链，在不影响其它数组链路情况下，把锁的粒度降低了很多

    v. 不安全：t1 线程进行扩容时，其它线程也可能在进行扩容，线程不安全。而且扩容方法 `addCount(1L, binCount);` 还不能简单的直接用 synchronized 加到方法处，因为这会非常影响性能，t1 线程在扩容时，别的线程就没办法 put 元素了，而且还还必须等扩容完成后插入新的数组才行，不然插入旧数组也没有意义。==所以必须要满足线程进行扩容时，其他线程都不要进行插入数据==
...
11. **有没有线程安全的并发容器？为什么要 ConcurrentHashMap？ConcurrentHashMap 是如何实现的？**
...
12. **TransferValue 值传递问题？**
    ...
    ![值传递问题](http://cdn.liumulin.top/interview/%E5%80%BC%E4%BC%A0%E9%80%92%E9%97%AE%E9%A2%98.jpg)
...
14. **<span id='newObject'>一个对象占用多少个字节？</span>对象的创建过程？**
    ...
    ![Java 对象头信息](http://cdn.liumulin.top/interview/Java%20%E5%AF%B9%E8%B1%A1%E5%A4%B4%E4%BF%A1%E6%81%AF.jpg)
...
16. 子父类（静态变量）执行顺序（待整理）

###  数据结构和算法

[找工作刷题，应该刷牛客网还是刷leetcode？](https://www.zhihu.com/question/317497607/answer/642442147)
...
1. **树**
   ...
   ![tree](http://cdn.liumulin.top/interview/tree.png)
...
[【要学会查看 javase 官方文档，博客都是互相抄】Java Platform Standard Edition 8 Documentation](https://docs.oracle.com/javase/8/docs/)
...
1. **JVM Architecture**
   ...
   ![Java-Heap-Space](http://cdn.liumulin.top/interview/2022/10/Java-Heap-Space.jpg)

   ![JVM-Architecture](http://cdn.liumulin.top/interview/2022/10/JVM-Architecture.jpg)

   一个进程包含着整个运行时数据区对应的是一个 JVM 实例，进程里面包括线程私有部分 `虚拟机栈`、`本地方法栈`、`程序计数器`，同时还有线程共享空间 `方法区` 和 `堆`

   JVM 里面由于线程时私有的而且默认只有 500 多到 1024k 大小，所以它不存在调优问题。调优一般都发生堆空间中

   - 栈（虚拟机栈）：就是我们平时就用的方法，每个方法就是一个栈帧

     > 如果代码里面进行递归调用的话，会导致栈满了发生 java.lang.StackOverflowError 异常

   - ==堆（重点优化）：==年轻代、年老代

     ![JVM-Heap-Memory-Metaspace](http://cdn.liumulin.top/interview/2022/10/JVM-Heap-Memory-Metaspace.png)

     ![Heap-Memory](https://miro.medium.com/max/1400/0*rzQ6-DyP-2gjiua7)
   ...
   - 程序计数器
     ...
     why：因为 CPU 需要不停的切换各个线程，切换回来的时候得知道接着从哪开始继续执行

     为什么设置为线程私有？

     还是为了准确地记录各个线程正在执行的当前字节码指令地址，切换回来的时候得知道接着从哪开始继续执行。

     > 本地接口、本地方法库。执行非 Java 代码，比如原子引用 AtomicInteger 底层就是 native 方法，用它的就是因为 C++ 底层是汇编，执行效率是最高的
...
3. **谈谈你对类加载器的理解**
   ...
   ![JVM-ClassLoader](http://cdn.liumulin.top/interview/2022/10/JVM-ClassLoader.jpg)
...
5. **父类双亲委派机制**
   ...
   - 如果一个类加载器收到了类加载请求，它并不会自己先加载，而是把这个请求委托给父类加载器执行
...
5. **请你谈谈 JVM 垃圾回收机制（Java 中如何判断一个对象是否是一个垃圾 | JVM 垃圾回收时如何确定垃圾 | 你知道有哪些垃圾回收算法吗？）**
   ...
   - 可达性分析算法
     ...
     ![可达性分析算法](http://cdn.liumulin.top/interview/2022/10/GCRoots.jpg)
   ...
   - <span id='fuzhiqingchu'>复制清除算法（Mark - Sweep）</span>：发生在年轻代，复制对象之后，谁空谁是 To 区
     ...
     ![GC-Mark-Sweep](http://cdn.liumulin.top/interview/2022/10/GC-Mark-Sweep.jpg)
   ...
   - 复制算法
     ...
     ![GC 复制算法](http://cdn.liumulin.top/interview/2022/10/20221026021159.jpg)

7. **你说你做过 JVM 调优和参数配置，请问如何盘点查看 JVM 系统默认值**

   查看一个 JVM 的初始值：

   jps：查看 java 进程 ID 和应用程序主类名（比如一个 main 方法进程启动）；`jps -l` 是查看进程 ID 包括全包名

   jinfo -flag `[xx参数]` `进程 ID`：查看某个 JVM 属性值是否开启或者查看出默认值是多少。因为有 Boolean 类型和 key、value 类型

   jinfo -flags `进程 ID`：查看当前能查看到的所有参数
   ...
   **查看参数盘点家底**
   ...
   java -XX:+PrintFlagsInitial：查看参数初始值

   java -XX:+PrintFlagsFinal：查看（修改后的）最终值（初始值可能被修改掉）

   java -XX:+PrintFlagsFinal -XX:MetaspaceSize=512m `jps 查看的类名` ：一边运行 java 程序一边动态修改参数的默认运行值（比如要修改元空间的默认值，当然也可以是改别的参数。原本默认的 uintx MetaspaceSize = 21807104（20m）改为了 uintx MetaspaceSize := 536870912（512m），改之前是 `=` 初始默认值，改之后是 `:=` 修改后的最终值

   java -XX:+PrintCommandLineFlags -version（重要）：查看初始参数（比较精简）。可以查看堆初始大小、最大大小。最后可以看到使用的是什么垃圾收集器
...
OpenJDK 64-Bit Server VM (build 25.292-b10, mixed mode)
   ...
   **参数综合运用**
   ...
   先来个 main 程序一直在后台运行，然后通过 jps 命令查看进程，jinfo -flag `[xx参数]` `进程 ID`：查看某个 JVM 属性值是否开启或者查看出默认值是多少
...
8. **你平时工作用过的 JVM 常用基本配置参数有哪些？（重要）**

   [【要学会查看 javase 官方文档，博客都是互相抄】Java Platform Standard Edition 8 Documentation](https://docs.oracle.com/javase/8/docs/)

   [【 JVM】Java 8 JVM 参数 Java Platform, Standard Edition Tools Reference for Oracle JDK on Windows, Release 8](https://docs.oracle.com/javase/8/docs/technotes/tools/windows/index.html)

   > 生产中 JVM 调优配置的参数（典型设置案例，不是标准的）：`Xms128m -Xmx4096m -Xss1024k -XX:MetaspaceSize=512m -XX：+PrintCommandLineFlags -XX：+PrintGCDetails -XX：+UseSerialGC`
   > 前六个参数非常重要，务必理解
   > JVM 调优：用的 undertow，压测后比 Tomcat 高出 3000 左右的 QPS，订单、物流等微服务 Xms、Xmx 设置的大一点 1024m，这两个参数调成一样大来避免 GC 频繁的收集。用的 G1 垃圾收集器来优化自动调节。元空间设置大一点改为 1024m，默认只有 20m 左右，频繁 new 一些对象的话就会撑爆

   - -Xms（ -XX:+InitialHeapSize）memory startup：初始化内存大小，默认为物理内存的 1/64

   - -Xmx（-XX:+MaxHeapSize）memory max：最大内存大小，默认为物理内存的 1/4
   
   - -Xss（-XX:+ThreadStackSize）stack size：单个线程栈大小，默认为 512k~1024k（有可能为 0 不要疑惑，具体的值是依赖于哪个平台，单个线程很小是线程私有的所以不需要垃圾回收），可以设置为 1024k，稍微设置大一点点可以减小报 StackOverflow 错误，也不能设置的太大
   
   - -Xmn：新生代大小
   
   - -XX:+MetaspaseSize：元空间大小，不管物理内存多大，查看它初始值只有 20m 左右，非常小导致频繁 new 一些对象的话就会撑爆元空间，所以可以把这个调大一些改为 512/1024m
   
     > 元空间的本质和永久代类似，都是对 JVM 规范中方法区的实现。不过元空间与永久代之间最大的区别在于<font color='red'>元空间并不在虚拟机中，而是使用本地物理内存</font>。因此，默认情况下元空间的大小仅受本地内存限制
   
   - -XX:+PrintGCDetails：打印详细 GC 日志信息
     ...
     public class HelloGC {
         public static void main(String[] args) throws InterruptedException {
             ...
             byte[] bytes = new byte[50 * 1024 * 1024]; // 初始设置的最大只有 10m 空间， 而 new 了一个 50m 对象，把堆空间撑爆了导致 OOM
         }
     }
     // ================================== print out ==================================
   ...
   - -XX:SuvivorRatio（弱）：修改 Eden 区的比例
   
     新生代（Young 区）中 Eden、s0 和 s1 空间的比例默认 - XX:SurvivorRatio=8， Eden:S0:S1 =8:1:1 
   
     设置 -XX:SurvivorRatio=4， Eden:S0:S1 = 4:1:1
   
   - -XX:NewRatio（弱）：配置年轻代与年老代在堆结构的占比
   
     默认 - XX:NewRatio=2 新生代占 1 老年代 2，年轻代占整个堆的 1/3
   
     > 一般 Young 区（年轻代）按默认配置不要修改，配置小了 GC 就会很频繁
   
     假如 - XX:NewRatio=4 新生代占 1，老年代 4，年轻代占整个堆的 1/5。NewRatio 值就是设置老年代的占比，剩下的 1 给新生代
   
     新生代 1/3 ，老年代 2/3
   
     > IDEA 中修改 VM options 并查看 GC 细节：-Xms10m -Xmx10m -XX:+PrintGCDetails -XX:+UseSerialGC -XX:NewRatio=2
   
   - -XX:MaxTenuringThreshold（弱）：设置垃圾最大年龄，也就是 From 和 To 之间移动的次数
   
     -XX MaxTenuring Threshold=0：如果设置为0的话，则年轻代对象不经过 Survivor 区，直接进入年老代。对于年老代比较多的应用，可以提高效率。
...
9. **（弱）Java 中的四种引用类型及其含义（强软弱虚引用的区别以及 GC 对他们执行怎样的操作）**
...
12. **请你谈谈 OOM？| 你工作中都遇到过哪些异常（是错误 ERROR）**

    ![the_superclass_of_all_errors_and_exceptions](http://cdn.liumulin.top/interview/the_superclass_of_all_errors_and_exceptions.jpg)
...
13. 哈哈哈
    ...
    ![Garbage-Collectors](http://cdn.liumulin.top/interview/2022/10/20221026022049.jpg)
  ...
  - 串行收集器
    ...
    - 年老代
      ...
      - ==并发标记清除收集器 CMS==
        ...
        CMS 收集器（Concurrent Mark Sweep：并发标记清除）是一种以获取最短回收停顿时间为目标的收集器。适合应用在互联网站或者B/S系统的服务器上，这类应用尤其重视服务器的响应速度，希望系统停顿时间最短。CMS 非常适合堆内存大、CPU 核数多的服务器端应用，也是 G1 出现之前大型应用的首选收集器。
...
15. **G1 垃圾收集器**
    ...
    CMS 垃圾收集器虽然减少了暂停应用程序的运行时间，但是它还是存在着内存碎片问题。于是，为了去除内存碎片问题，同时又保留CMS垃圾收集器低暂停时间的优点，也就是 STW 时间更可控，G1在停顿时间上添加了预测机制，用户可以指定期望停顿时间，Java 7 发布了一个新的垃圾收集器-G1垃圾收集器。JDK9 中 G1 已经成为了默认收集器
    
    特点：
    
    1：G1 能充分利用多 CPU、多核环境硬件优势，尽量缩短 STW
    2：G1 整体上采用标记整理算法，局部是通过复制算法，不会产生内存碎片。
    3：宏观上看 G1 之中不再区分年轻代和老年代。把内存划分成多个独立的子区域（Region），可以近似理解为一个围棋的棋盘。
    4：G1 收集器里面讲整个的内存区都混合在一起了，但其本身依然在小范围内要进行年轻代和老年代的区分，保留了新生代和老年代，但它们不再是物理隔离的，而是一部分 Region 的集合且不需要是连续的，也就是说依然会采用不同的 GC 方式来处理不同的区域。
...
17. **假如生产环境出现 CPU 占用过高，请谈谈你的分析思路和定位？ | 谈谈你记忆深刻的故障**
    ...
    - 内存占用过高排查流程
      - 查找进程 ID: 利用 `top -d 2 -c` 命令查出占最高的的进程 pid ，如果 pid 为 9876
      - 用 jmap 来抓取内存快照
      - 查看 JVM 堆内存分配情况：jmap -heap pid
...
19. **JVM优化**
    ...
    4：系统停顿的时候可能是 GC 的问题也可能是程序的问题，多用 jmap 和 jstack 查看，或者 killall -3 Java，然后查看 Java 控制台日志，能看出很多问题。有一次网站突然很慢，jstack 一看，原来是自己写的 URLConnection 连接太多没有释放
    ...
    >
    > 1. 如何观察JVM的运行参数。比如你发现Java程序跑得慢，定期卡顿，OOM挂掉。为了找原因，你必须学会怎么去看JVM到底怎么运行的。这时需要你了解一些JVM自带的工具命令，比如jps、jstack、jhat，还有比较新的Java自带的JMC图形界面工具。通过他们你可以了解到一个Java进程有多少线程，每个线程什么状态，是不是在等着锁：进程的CPU和内存占用了多少；GC状态怎么样，Full GC频繁不频繁等等；内存中是不是有泄露，哪个地方可能泄漏了等等。
    ...
    >
    > 在面试中可以先从一个具体的问题开始讨论你的做法，从猜测，到运用工具观察，到找到问题所在，最终提出方案。当然前提是你的确熟悉这些内容。如果在实际工作中真的没尝试过，可以看看这本书《[深入理解Java虚拟机](https://book.douban.com/subject/6522893/)》混个脸熟。这本书里有数个调优的例子，值得参考。
    >
    > 作者：大宽宽
    > 链接：https://www.zhihu.com/question/268821097/answer/342151672
    > 来源：知乎
    ...
    >
    > 这两层是主流在做的事情，后面就是玄学了也不用分层了。。。什么重排序啊，可见性啊，不同类加载器啊，多工具联合监控啊，热部署介入啊，这些反正面试当中也不会问的。。。。
    >
    > 
    >
    > 作者：萝魏紫
...
![Java 主流锁](http://cdn.liumulin.top/interview/Java%20%E4%B8%BB%E6%B5%81%E9%94%81.png)
...
5. **线程的状态都有哪些？（线程的生命周期）**
   ...
   ![线程的生命周期](http://cdn.liumulin.top/interview/%E7%BA%BF%E7%A8%8B%E7%9A%84%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F.jpg)
...
6. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   > 主内存是共享内存区域。由于工作内存是线程的私有区域，因此线程之间无法访问对方的工作内存；由于不能直接操作主内存中的变量，线程之间的通讯（传值）必须通过主内存来完成，工作内存存的是共享变量副本
     ...
     ![JMM 内存模型](http://cdn.liumulin.top/interview/JMM%20%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B.jpg)
   ...
   **volatile 是 JVM 提供的比 synchronized 轻量级的同步机制，它有三大特性：保证内存的可见性、禁止指令重排序、但不保证原子性。**
   ...
   - 可见性
     ...
     ![JMM 内存可见性](http://cdn.liumulin.top/interview/JMM%20%E5%86%85%E5%AD%98%E5%8F%AF%E8%A7%81%E6%80%A7.jpg)
   ...
   - 不保证原子性：
     ...
     ![volatile 不保证原子性](http://cdn.liumulin.top/interview/volatile%20%E4%B8%8D%E4%BF%9D%E8%AF%81%E5%8E%9F%E5%AD%90%E6%80%A7.jpg)
...
7. **讲一讲 Atomiclnteger，为什么要用 CAS 而不是 synchronized？（CAS知道吗？如何实现的？CAS有什么缺陷，该如何解决）**
   ...
   ![CAS 解决 volatile 的原子性问题](http://cdn.liumulin.top/interview/CAS%20%E8%A7%A3%E5%86%B3%20volatile%20%E7%9A%84%E5%8E%9F%E5%AD%90%E6%80%A7%E9%97%AE%E9%A2%98.jpg)
   ...
   **CAS 缺点：**CPU - 共享变量 - ABA 问题
   ...
   3. **ABA 问题：**首先是先把数据拷贝到 T1、T2 线程的工作内存，CAS 在操作值的时候会去检查内存值是否发生变化，没有发生变化才会更新内存值。比如内存值原来是 A，后来变为了 B，接着又变为了 A，然后 T1 线程挂起，T2 线程进行 CAS 比较时发现内存值没有发生变化，然后改为最新值 C。但是实际上内存值是有变化的，这就是 CAS 的 ABA 问题。
      ...
      ![ABA 问题](http://cdn.liumulin.top/interview/ABA%20%E9%97%AE%E9%A2%98.jpg)
...
8. **悲观锁 VS 乐观锁，请讲一下它们的优缺点**
   ...
   ![悲观锁 VS 乐观锁](http://cdn.liumulin.top/interview/%E6%82%B2%E8%A7%82%E9%94%81%20VS%20%E4%B9%90%E8%A7%82%E9%94%81.png)
...
9. **谈谈你对 synchronized 关键字的理解（synchronized关键字的用法，优缺点 | synchronized 和 Lock 的区别）**
   ...
   > 助记：重 互 简 长 低 高 减。重要的人总是互相简单比手长、比脂肪低、比身高、比减肥

   在 JDK 早期的实现是重量级锁，底层是依赖于操作系统的互斥锁（Mutex Lock）。但如果同步代码块里面的代码非常简单，那 synchronized 去申请锁的时间就会比代码执行的时间还要长（比如+1操作），这就是为什么 JDK 6 以前 synchronized 效率低的原因。由于后来 Java 越来越多的开始处理高并发程序。于是从 1.6 开始 JDK 为了减少申请锁和释放锁带来的性能消耗就引入了 “偏向锁” 和 “轻量级锁”，这才有了锁升级的概念
   ...
   ![synchronized 锁升级](http://cdn.liumulin.top/interview/synchronized%20%E9%94%81%E5%8D%87%E7%BA%A7.jpg)
...
>
    ...
    - what：是用来构建锁和其他同步组件的基本框架。通过 CLH 变种的双端队列来完成资源获取和线程的排队工作，并通过一个 state 变量表示持有锁的状态（1 代表持有锁）
    
    - why 最重要的基石：ReentrantLock、CountDownLatch、ReentrantReadWritelock、Semaphore、CyclicBarrier 都和 AQS 有关
    ...
    - 底层源码：
      ...
      ![AQS](http://cdn.liumulin.top/interview/AQS.jpg)
    
      i. 假如有 T1、T2、T3 线程，以 ReentrantLock 为切入点，在调用 `lock()` 时默认会走非公平锁，T1 线程第一次进来会走 `lock()` ，通过 CAS 的方式把 volatile 修饰的 state 状态改为 1，并把 Thread 对象线程设置为 T1
    
      ii. 这时 T2 线程也想要获取锁，但进来之后发现 state 已经被占用，就只能走 `acquire()` 分支方法里的 `tryAcquire()` ，这个抽象方法什么都没实现还抛了异常，强行让子类去实现。所以就走到了非公平锁的方法 `nonfairTryAcquire()`，但是在进行判断之后发现当前 state 状态不是 0 而且正在占用的线程也不是 T2 线程，就会返回 false
    
      iii. 所以要接着往下调用 `addWaiter(Node.EXCLUSIVE)` 进行准备入队，实际是走到 `enq()` ，方法里面会循环的执行，首次会 new 一个空的哨兵节点（占位），Node 节点会有几个属性 waitStatus、Thread、next、prev。第二次循环的时候才是真正的让 T2 节点入队，然后把 tail 指向 T2，之后 T3 线程也同理
    
      iv. 接着在 `acquireQueued(addWaiter(Node.EXCLUSIVE), arg)` 进行自旋，去执行 `抢占失败是否 park()`  ，T2 线程第一次去进行抢占，如果抢占成功就去执行业务，抢占失败就把哨兵节点的 waitStatus 改为 -1。因为自旋嘛就还需要再去抢，如果还没成功，此时就会把 T2 进行 `park()` 阻塞，需要等待其他线程唤醒，T3 线程也同理
      
      v. ReentrantLock 调用 `unlock()` 进行释放锁，窗口的 Thread 对象变为 null，state 恢复为 0。接着把哨兵节点的 waitStatus 从 -1 改为 0，此时 T2 线程会被 `unpark()` 唤醒，最终抢占到窗口位置开始处理业务，这样就完成了出队操作
      
      vi. 最后需要处理的就是 GC 问题，队列中会把 head 指向 T2 节点，把 T2 节点的 Thread 设为 null，并且把 next、prev 指针指向 null。这样没有被引用指针指向的哨兵节点慢慢的就会被 GC 回收掉。由于 T2 线程办理出队业务去了，所以队列中的 T2 节点也就成了新的哨兵节点来进行占位
    ...
    final void lock() {
      ...
      - tryAcquire(arg)
        ...
        acquire()：第二个线程只能走 `acquire()` 分支了里的 `tryAcquire()` 方法
        ...
        父类代码直接抛异常，没有实现代码。这是典型的模板方法，父类定义的足够高，落地的实现放到子类，逼着必须你在子类进行实现
        ...
        final boolean nonfairTryAcquire(int acquires) {
            ...
            // 有可能是 T1 线程占用了两次，就是可重入锁，state 就会设为 2
...
13. **CountDownLatch、CyclicBarrier、Semaphore 使用过吗？**
    ...
    - CountDownLatch
      ...
      ![CountDownLatch](http://cdn.liumulin.top/interview/CountDownLatch.jpg)
...
15. **谈谈你对线程池的理解（线程池的工作原理，几个重要参数，然后给了具体几个参数分析线程池会怎么做，最后问阻塞队列的作用是什么？）**
    ...
    - **线程池如何使用？**
      ...
      ![线程池UML](http://cdn.liumulin.top/interview/%E7%BA%BF%E7%A8%8B%E6%B1%A0UML.jpg)
    ...
    - **ThreadPoolExecutor 类 7 大参数的含义：**
      ...
      > 助记：Cartier makes Kate.Perry unbelievable，with two high jewelry
    ...
    - **说说线程池的底层工作原理？(重要）**
      ...
      1. 在创建了线程池之后，线程池会等待提交过来的请求任务。当调用 `execute()` 方法时就会添加一个请求任务，此时线程池会做判断，如果正在运行的线程数量 < corePoolSize 时，是的话会立马创建线程来执行这个任务
      ...
      4. 非核心线程也满了的话那么线程池会执行拒绝策略（正在运行的线程数还 >=  maximumPoolSize）

      5. 当一个核心线程完成一个任务后，它会从阻塞队列中获取下一个任务来执行

      6. 当一个线程空闲了并且时间超过设置的 keepAliveTime 时，线程池就会判断，如果当前运行的线程数 > corePoolSize，那么这个线程就被停掉。当线程池执行完所有任务后最终把线程容量就会收缩到 corePoolSize 的大小

    
    ![线程池底层工作原理](http://cdn.liumulin.top/interview/%E7%BA%BF%E7%A8%8B%E6%B1%A0%E5%BA%95%E5%B1%82%E5%B7%A5%E4%BD%9C%E5%8E%9F%E7%90%86.jpg)
...
17. **死锁编码及定位分析**
    ...
    <img src="http://cdn.liumulin.top/interview/%E6%AD%BB%E9%94%81.jpg" alt="死锁" style="zoom:33%;" />
...
2. **什么是索引？索引的优缺点，什么字段上建立索引**
   ...
   **索引的分类**
   ...
   - 唯一索引：索引列的所有值都只能出现一次，即必须唯一，值可以为空。
     ...
     > 防止新增脏数据。比如：支付宝的资金账户，支付宝也有用户账户，每个用户只能有一个资金账户，怎么防止给用户创建多个资金账户，那么给资金账户表中的用户加唯一索引，所以一个用户只能新增成功一个资金账户记录。要点：唯一索引或唯一组合索引来防止新增数据存在脏数据（当表存在唯一索引，并发时新增报错时，再查询一次就可以了，数据应该已经存在了，返回结果即可
...
6. **SQL优化**
   ...
   vi. 强制类型转换时会造成全表扫描（varchar 类型没有加单引号的话会导致索引失效；而且还会导致行锁变表锁）
...
14. **SQL 题**
     ...
     [牛客网 SQL 篇](https://www.nowcoder.com/exam/oj?page=1&tab=SQL篇&topicId=199)

     [LeetCode 精选数据库 70 题](https://leetcode.cn/problem-list/qgq7m9e/?page=1)

15. [586. 订单最多的客户 - 力扣（LeetCode）](https://leetcode.cn/problems/customer-placing-the-largest-number-of-orders/)
...
17. [获取每个部门中当前员工薪水最高的相关信息_牛客题霸_牛客网](https://www.nowcoder.com/practice/4a052e3e1df5435880d4353eb18a91c6?tpId=82&&tqId=29764&rp=1&ru=/ta/sql&qru=/ta/sql/question-ranking)
...
[常用的设计模式汇总，超详细！](https://mp.weixin.qq.com/s?__biz=MzI4OTA3NDQ0Nw==&mid=2455546011&idx=1&sn=59c711e994e92b56dc0da3e520cba4ed&chksm=fb9cb4fbcceb3ded38d557bc90955f155f809975f024e2f83a6fda1d98a52a9c11f638483376&mpshare=1&scene=1&srcid=#rd)
...
1. **谈谈分布式事务（分布式事务的控制）**
   ...
   2PC（两阶段提交）：分为准备阶段和提交/回滚阶段，两个参与者往协调者提交sql，相当于彩排，协调者收集完两个服务的执行反馈之后再决定提交还是回滚。假如订单服务调用库存服务，库存服务出错了，协调者知道后就会喊订单服务回滚，这样就保证了分布式事务的一致性

   缺点：

   阻塞：1PC，准备阶段，只执行 sql，而不提交，并且占用数据库连接资源

   单点故障

   解决方案：手动补偿，脚本补偿（定时任务？），服务 1：数据修改 0->1，服务 2：数据修改 A->B。那么正确的数据内容为，服务1数据为0对应服务2数据为A；服务1数据为1对应服务2数据为B。否则就是错误数据需要脚本修改

   3PC（三阶段提交）：

   

   Two-PC 实现方式：等两个都执行完才提交，假提交到事务管理器。有一个失败就回滚 （两阶段提交模式）
...
3. **什么是微服务？为什么使用微服务（微服务优缺点）？**
   ...
   按照 Martinflower 博客描述的，微服务架构风格是一种将单个应用程序开发为一组微服务的方法，每个微服务都运行在自己的进程中并采用轻量级机制（通常是 HTTP 资源 API）进行通信。这些服务是围绕业务能力构建的，并且可以通过全自动部署机制独立部署。并且这些服务可以用不同的编程语言编写并使用不同的数据存储技术。
...
5. **CAP 定理（重要）**
   ...
   C：一致性（Consistency），数据在多个副本节点中保持一致，可以理解成两个用户访问两个系统 A 和 B，当 A 系统数据有变化时，及时同步给系统，让两个用户看到的数据是一致的。

   A：可用性（Availability），系统对外提供服务必须一直处于可用状态，在任何故障下，客户端都能在合理时间内获得服务端非错误的响应。工

   P：分区容错性（Partition tolerance），在分布式系统中遇到任何网络分区故障，系统仍然能对外提供服
   务。网络分区，可以这样理解，在分布式系统中，不同的节点分布在不同的子网络中，有可能子网络中只有一个节点，在所有网络正常的情况下，由于某些原因导致这些子节点之间的网络出现故障，造成整个节点环境被切分成了不同的独立区域，这就是网络分区。

   - 当网络发生故障时，系统 A 和系统 B 没法进行数据同步，也就是我们不满足 P，同时两个系统依然可以访问，那么此时其实相当于是单机系统，就不是分布式系统了，所以既然我们是分布式系统，必须满足。
   - 当 P 满足时，如果用户 1 通过系统 A 对数据进行了修改将 data1 改成了 data2，也要让用户 2 通过系统 B 正确的拿到 data2，那么此时是满足 C，就必须等待网络将系统 A 和系统 B 的数据同步好，并且在同步期间，任何人不能访问系统 B（让系统不可用），否则数据就不是一致的。此时满足的是 CP。
   - 当 P 满足时，如果用户 1 通过系统 A 对数据进行了修改将 data1 改成了 data2，也要让系统 B 能继续提供服务，那么此时，只能接受系统 A 没有将 data2 同步给系统 B（牺牲了一致性）。此时满足的就是 AP

6. **CAP（请说下 Redis/eureka 和 zookeeper 两个的区别）**

   CAP 理论：CAP 理论核心是一个分布式系统不可能同时很好满足一致性、可用性、分区容错性这三个需求

   Redis 保证的是 AP：Redis 集群部署时，当一条数据到达 Redis，首先会回复 OK，再把数据异步复制给从节点。它提高了并发能力，但牺牲了数据一致性

   > Eureka 保证的是 AP：Eureka 在设计时就先保证了可用性，节点间都是平等的，节点挂掉不会影响其它节点的工作，Eureka 的客户端在向某个 Eureka 注册服务时如果失败，则会自动切换至其它节点

   Zookeeper 保证的是 CP：它是把主节点的数据同步给 slave 节点，保证所有的数据都同步了才返回 OK。这样 Zookeeper 保证了数据一致性，但并发性就下降了
...
### 框架相关 Spring、SpringMVC、MyBatis 等等

[Spring这么简单你竟然不会？深入解读Spring基础、IOC、AOP、源码、面试题_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1mT4y1o7v5/?vd_source=4844de7cb051be29fbaf4555af0bbd8b)

源码必学（1. 目的就是能够为所欲为 2. 读项目中同事写的中间件等，难道他们比 Spring 的人还牛？）

方法论：1、先梳理脉络，然后再抠细节2、不要忽略注释3、学会使用 IDEA 小工具 translate4、见名知意、5、大胆猜测，小心验证6、坚持看

> 脉络图已经有了，按照图再死扣细节，给自己指定一个规划，一天看一个方法流程（今天加载配置文件方法，明天填充属性方法）。最后需要总结一个 Spring 的全体系流程图

Spring 一般考察：IOC、AOP、TX 事务
...
初中级岗位你就一直往下聊就行了（避重就轻）。高级岗位有经验的面试官可能会打断你说聊聊我感兴趣的东西

一个问题能占用面试官多少时间？问的越多可能露馅越多

当面试官问到一个你熟悉的点的时候，一定要尽量拖时间。不熟悉的就一笔带过不让他问了

面试官可能也记不住，面试官也是人，凭什么他记得住，我记不住
...
2. **谈谈 Spring IOC 的理解，原理和实现**

   [【尚硅谷】Spring框架视频教程（spring5超详细源码级讲解）_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Vf4y127N5/?vd_source=4844de7cb051be29fbaf4555af0bbd8b)
   ...
   > xxPostProcessor（后置处理器 | 增强器）：要立马反应到是做增强功能的
   >
   > Environment 里的 `system.getEnv` `system.getProperties` 就是程序启动时加载进了容器，我们可以直接获取 CPU 核数等

   - what：

     - 控制反转 (Inversion of Control)

       理论思想：原来的对象是由使用者来进行控制；而有了 Spring 容器之后可以把整个对象交给Spring 来帮我们管理

       Dl（Dependency Injection）：依赖注入把属性值注入到对象中，`@Autowired`，`populateBean()` 都是用来完成属性值注入的

     - 容器

       是用来存储对象，我们在容器中一般都是根据名称来获取的 bean 对象的，所以里面肯定是用 map 结构。一般用三级缓存 singletonObjects 来存放完整的 bean 对象。bean 的整个生命周期，从创建到使用到销毁的过程全部都交给了 Spring 容器来管理

   - why：降低耦合度

   - how（原理 | 源码 `refresh()`）：

     ![Spring_IOC](http://cdn.liumulin.top/interview/Spring_IOC.jpg)

     我用图来稍微表述一下整个过程：

     i. 首先有一个 Container 容器，当我们需要访问容器的时候 Spring 提供了一个访问容器的根接口 BeanFactory。默认是 `new DefaultListableBeanFactory()`

     ii. 如果要让 Spring 帮我们创建对象，那么就需要在配置文件中定义好 bean 信息。要把 xml 配置文件（或者 properties、yaml以及注解等方式）进行加载、解析变成一个 BeanDefinition 对象（bean 的定义信息）。但是这么多种配置文件转换的方式肯定不一样，假如后面还想出一直 JSON 文件来定义 bean 对象呢？那么这时候就需要考虑定义一种规范来方便扩展，所以 Spring 在前面加了一个抽象的接口 BeanDefinitionReader，相当于给出一个固定的接口让不同的子类去实现就可以了（相当于定义了一个规则）。

     iii. 有了 bean 定义信息之后接着该实例化对象，实例化操作可以通过 new、反射以及工厂等方式来实现。但如果这样就结束了的话岂不是太简单了？太简单用户就没法去自定义一些东西了。而且 Spring 框架它是 SpringBoot 和 SpringCloud 等框架的基石，所以它需要有最重要的一个特点 —— 扩展。所以在 bean 实例化前会有多个扩展点，我们可以通过实现 BeanFactoryPostProcessor 来做一些扩展功能，比如 Spring 里的 PlaceholderConfigurerSupport 类就是通过实现 BeanFactoryPostProcessor 接口来对占位符做替换处理的一个扩展功能（数据库连接信息用户名、密码进行占位符的替换）

     iv. 所有扩展点执行完之后就该实例化对象了，可以反射来实例化对象，核心代码就是 `Constructor ctor = Clazz.getDeclareConstructor();` + `Object obj = ctor.newInstance()`

        > 获取 Class 的三种方式：`Class.forName(xxx)` `类.Class` `对象.getClass()`

     v. 然后开始初始化对象，也就是 bean 的生命周期

     - 填充属性：调用 `populateBean()` 填充属性

     - 调用 Aware 接口的方法：如果我们想要在自定义对象中注入 Spring 容器的内置对象 BeanFactory、ApplicationContext 怎么办呢？如果显式的调用肯定很麻烦，可以通过实现 BeanFactoryAware、ApplicationContextAware 来让 Spring 容器自己调用  `invokeAwareMethods()`  来帮我们完成注入

     - before 和 after：针对的是 BeanPostProcessor，而 BeanPostProcessor 扩展针对的是 Bean 对象，而前面的 BeanFactoryPostProcessor 扩展是针对与 BeanFactory 的，那么就可以在 before、after 位置对 bean 进行 AOP 扩展，AOP 的时候底层是动态代理（有 JDK、Cglib 两种方式），但在实现 AOP 之前必须要有 bean 对象，因为它的内部代码对 bean 是做了判空操作的，这个时候要扩展的话就要用到专门扩展 Bean 对象 BeanPostProcessor 接口，用的是 AbstractAutoProxyCreator 来实现的 BeanPostProcessor 接口，里面有前置和后置（before 和 after）初始化方法来实现动态代理

     vi. 获取到完整的对象

     > 动态代理选择源码位置：BeanPostProcessor、AbstractAutoProxyCreator.postProcessAfterInitialization()、wrapIfNecessary()、createProxy()、getProxy()、getProxy()

     ```json
     容器和对象的创建流程：
     1、先创建容器
     2、加载配置文件，封装成BeanDefinition
     3、调用 invokeBeanFactoryPostProcessors(beanFactory) 方法执行 BeanFactoryPostProcessor
     实例化前的准备工作：
     	准备beanPostProcessor
     准备监听器，事件，广播器 // 这里是为了打印出日志，比如打印 bean 属性填充完成、
     4、实例化
     5、初始化
     6、获取到完整对象
     ```
     > i. 首先是容器就需要创建容器工厂，容器上层有个 BeanFactory 的根接口，没有提供子类实现，我们实际调用中最常用的就是 DefaultListableBeanFactory，然后会向 bean 工厂中设置一些参数（像 BeanPostProcessor、Aware 接口的子类）等等属性
     >
     > ii. 加载解析 bean 对象，准备要创建 bean 对象的定义对象 beanDefinition，（xml 或者注解的解析过程）
     >
     > iii. beanFactoryPostProcessor 的处理，这里是扩展点，Spring 为什么有这么好的生态发展的那么好就是因为 Spring 提供了很多扩展点，比如常见的 PlaceHolderConfigureSupport（占位符处理），ConfigurationClassPostProcessor（BeanPostProcessor 的注册功能，方便后续对 bean 对象完成具体的扩展功能
     >
     > iv. 通过反射的方式讲 BeanDefinition 对象实例化成具体的 bean 对象
     >
     > v. bean 对象的初始化过程（填充属性，调用 aware 子类的方法，调用 BeanPostProcessor 前置处理方法，调用 init-mehtod 方法，调用 BeanPostProcessor 的后置处理方法）
     >
     > vi. 生成完整的 bean 对象，通过 getBean 方法可以直接获取生成完整的 bean 对象，通过 getBean 方法可以直接获取
     >
     > vii. 销毁过程

     i. 首先是容器就需要创建容器工厂，容器上层有个 BeanFactory 的根接口，实际创建的时候是 `new DefaultListableBeanFactory()` 

     ii. 通过 `loadBeanDefinitions(beanFactory)` 加载配置文件的 Bean 定义信息（放到 map 中）

     > `prepareBeanFactory(beanFactory)` 进行设置属性值的前戏操作

     iii. 调用 `invokeBeanFactoryPostProcessors(beanFactory)` 进行功能扩展

     > 比如常见的 PlaceHolderConfigureSupport（占位符处理），调用完 `invokeBeanFactoryPostProcessors(beanFactory)` 之后数据库 bean 对象的 username、password 就已经替换了占位符并且有值了，所以我们也可以自己来实现 `BeanFactoryPostProcessor` ，然后从 BeanFactory 中拿到 bean 对象进行修改

     iv. 通过反射实例化对象 `clazz.getDeclaredConstructor();` + `ctor.newInstance (argsWithDefaultValues);`，调用的无参构造

     v. 调用 `populateBean()` 进行属性值填充

     > 面试官，这是我对 IOC 的整体理解，包含了一些详细的处理过程，您看一下有什么问题，可以指点我一下（允许你把整个流程说完）
>
     > 老师，我没看过源码怎么办？
>
     > 具体的细节我记不太清了，但是 Spring 中的 bean 都是通过反射的方式生成的，同时其中包含了很多的扩展点，比如最常用的对 BeanFactory 的扩展，对 bean 的扩展，我们在公司对这方面的使用是比较多的，除此之外，IOC 中最核心的也就是填充具体 bean 的属性，和生命周期（背一下）
>
     > PostProcessor：BeanPostProcessor 针对的是 BeanFactory 对象；BeanPostProcessor 针对的是 bean 对象
     >
     > Spring 默认是单例的，保证从容器中只会获取到一个对象，那么每次创建对象之前就会先去容器里面查找，找不到才创建
     >
     > 需要代理的对象要不要创建一个空的对象？
     >
     > 要，后面有判断逻辑

3. **谈一下 Spring IOC 的底层实现**

   工厂 + 反射 + 设计模式

   关键方法：createBeanFactory、getBean、doGetBean、createBean、doCreateBean、createBeanInstance（getDeclaredConstructor、newInstance 创建实例的）、populateBean（填充属性的）、initializingBean

   i. 先通过 createBeanFactory 创建出一个 Bean 工厂（DefaultListableBeanFactory）

   ii. 开始循环创建对象，因为容器中的 bean 默认都是单例的，所以优先通过 getBean,doGetBean 从容器中查找，如果找不到的话

   iii. 通过 createBean,doCreateBean 方法，以反射的方式创建对象，一般情况下使用的是无参的构造方法（getDeclaredConstructor，newlnstance）

   iv. 进行对象的属性填充 populateBean

   v. 进行其他的初始化操作（initializingBean）

4. **Spring 是如何解决循环依赖问题的**

   [【阳哥】37_spring循环依赖debug源码03_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Hy4y1B78T?p=38&spm_id_from=pageDriver&vd_source=4844de7cb051be29fbaf4555af0bbd8b)
   ...
   总 what：A 依赖 B，B 依赖 A（A 对象里面有个 b 属性，B 对象里面有个 a 属性，形成了一个闭环）
   ...
   // 构造方法注入会产生循环依赖，一直需要 new 另一个，不然会报错
   ...
   默认 Spring 配置的 scope 是 singleton 的话不会保存，改为 prototype 之后会报 BeanCurrentlyInCreationException

   只有单例的 bean （scope 配置为 singleton）会通过三级缓存提前暴露来解决循环依赖的问题；而非单例的 bean（scope 配置为 prototype），每次从容器中获取都是一个新的对象，都会重新创建，所以非单例的 bean 是没有缓存的，不会将其放到三级缓存中。

   在 `DefaultsingletonBeanRegistry` 类中有三个 map

   一级缓存 singletonObjects：单例池，用来存放经历了完整生命周期的 bean 对象

   二级缓存 earlySingletonObjects（早期暴露的）：用来存放早期暴露出来的 bean 对象（实例化但未初始化的 bean），bean 生命周期还没结束（属性还未填充的对象）

   三级缓存 singletonFactories：存放可以生成 bean 的工厂（用于生产）；存放的是 FactoryBean，假如 A 类实现了 FactoryBean，那么依赖注入的时候不是 A 类，而是 A 类产生的 bean

   阳哥版本：

   i. Spring 创建 bean 主要分为两个步骤，实例化和初始化，实例化就是在堆中开辟空间，属性还是默认值，初始化才会进行属性填充赋值操作（创建原始 bean 对象，接着去填充对象属性和初始化）

   ii. 每次创建 bean 之前，我们都会从缓存中查下有没有该 bean，因为是单例，只能有一个

   iii. 当我们创建 beanA 的原始对象后，并把它放到三级缓存中，然后填充对象属性。这时发现依赖了 beanB，接着就又去创建 beanB，同样的流程，创建完 beanB 填充属性时又发现它依赖了 beanA 又是同样的流程。

   但不同的是这时可以在三级缓存中查到刚放进去的原始对象 beanA，所以不需要继续创建，于是把 beanA 注入 beanB，完成 beanB 的初始化（完整创建）

   iv. 既然 beanB 创建好了，所以 beanA 就可以完成填充属性的步骤了，接着执行剩下的逻辑，闭环完成

   代码：

   i. 调用 `doGetBean()` 想要获取 beanA，于是调用 `getSingleton()` 从缓存中查找 beanA

   ii. 在 `getSingleton()` 中，从一级缓存中查找，没有，返回 null

   iii.  `doGetBean()` 中获取到的 beanA 为 null，于是走对应的处理逻辑，调用  `getSingleton()` 的重载方法（参数为 ObjectFactory 的）

   iv. 在 `getSingleton()` 中，先将 beanA_name 添加到一个集合中，用于标记该 bean 正在创建中。然后回调匿名内部类的 `createBean()`

   v. 进入 AbstractAutowireCapable BeanFactory#doCreateBean，先反射调用构造器创建出 beanA 的实例，然后判断：是否为单例、是否允许提前暴露引用（对于单例一般为 true）、是否正在创建中（即是否在第四步的集合中）。判断为 true 则将 beanA 添加到【三级缓存】中

   vi. 对 beanA 进行属性填充，此时检测到 beanA 依赖于 beanB，于是开始查找 beanB

   vii. 调用 `doGetBean()` ，和上面 beanA 的过程一样，到缓存中查找 beanB，没有则创建，然后给 beanB 填充属性

   viii. 此时 beanB 依赖于 beanA，调用 getSingleton（获取 beanA，依次从一级、二级、三级缓存中找，此时从三级缓存中获取到 beanA 的创建工厂，通过创建工厂获取到 singletonObject，此时这个 singletonObject 指向的就是上面在 `doCreateBean()` 中实例化的 beanA

   ix. 这样 beanB 就获取到了 beanA 的依赖，于是 beanB 顺利完成实例化，并将 beanA 从三级缓存移动到二级缓存中，然后并把三级缓存的 beanA 删除

   x. 随后 beanA 继续他的属性填充工作，此时也获取到了 beanB、beanA 也随之完成了创建，回到 `getSingleton()` 方法中继续向下执行，将 beanA 从二级缓存移动到一级缓存中

   连鹏举老师版本：

   i. 先创建 A 对象，实例化 A 对象，此时 A 对象中的 b 属性为空，填充属性 b

   ii. 从容器中查找 B 对象，如果找到了，直接赋值不存在循环依赖问题（不通），找不到直接创建对象

   iii. 实例化 B 对象，此时 B 对象中的 a 属性为空，填充属性 a

   iv. 从容器中查找 A 对象，找不到，直接创建

   导致形成了一个闭环，所以关键点就是要打断实例化和初始化操作

   形成闭环的原因：此时，如果仔细琢磨的话，会发现 A 对象是存在的，只不过此时的 A 对象不是一个完整的状态，只完成了实例化但是未完成初始化，如果在程序调用过程中，拥有了某个对象的引用，能否在后期给他完成赋值操作，可以优先把非完整状态的对象优先赋值，等待后续操作来完成赋值，相当于提前暴露了某个不完整对象的引用，所以解决问题的核心在于实例化和初始化分开操作，这也是解决循环依赖问题的关键

   当所有的对象都完成实例化和初始化操作之后，还要把完整对象放到容器中，此时在容器中存在对象的几个状态，完成实例化但未完成初始化状态，完整状态，因为都在容器中，所以要使用不同的 map 结构来进行存储，此时就有了一级缓存和二级缓存，如果一级缓存中有，那么二级缓存中就不会存在同名的对象，因为他们的查找顺序是 1,2,3 级这样的方式来查找的

   为什么需要三级缓存？三级缓存的 value 类型是 ObjectFactory，是一个函数式接口，存在的意义是保证在整个容器的运行过程中同名的 bean 对象只能有一个。
...
5. **BeanFactory 与 FactoryBean 有什么区别？**
   ...
   不同点：BeanFactory 创建对象需要严格遵循生命周期，比较复杂。如果想要简单自定义某个对象的创建，我们可以自己实现 FactoryBean，可以在方法里面直接 new 操作而不走一遍生命周期。（FeignClient 就是这么做的）

6. **Spring中使用了哪些设计模式**

   说前四个就好了

   - 单例模式：一个类在内存中只有一个对象实例
   ...
   - 工厂模式：通过解析全类名来动态创建类，像 BeanFactory 等容器都有用到

   - 模板方法模式：用来解决代码重复的问题，通过抽象类把方法定义的足够高，具体实现由子类来完成

     Spring 源码里的 `AbstractApplicationContext.postProcessBeanFactory()` 和 `onRefresh()` 方法就是模板方法的体现，里面什么都没实现只是留给子类扩展使用的

     > Spring 中的 BeanFactory 就是简单工厂模式的体现，根据传入一个唯一的标识来获得 Bean 对象，不过是否在传入参数后创建还是传入参数前创建这个要根据具体情况来定
   ...
   - 观察者模式：listener、event、muticast。有点像发布订阅模式，发布者发布消息，订阅者那边获取消息
...
7. **Spring AOP 的底层实现原理？**
   ...
   what：面向切面编程 (Aspect-Oriented Programming)，它只是 IOC 的一个扩展功能点 （一个BeanPostProcessor），我们通过它能够在不修改源码的情况下对功能进行增强。一般应用于系统中的日志、事务等（与业务无关的）场景上，还有就是做接口幂等防刷

   > 幂等防刷：自定义注解+防刷的切面类，在处理前进行防刷

   分：

   > 助记：b-代-J-字节码-通知-拦，bean代表JJ叫字节码通知拦截器链

   大致的流程呢就是

   i. bean 的创建过程中有个步骤可以对 bean 进行扩展，AOP 本身就是一个扩展功能，所以会在 BeanPostProcessor 的后置处理方法中进行实现

   ii. 代理对象的创建过程，会先获取 advice、切面以及切点等信息

   iii. 通过 JDK 或 Cglib 的方式来生成对象

   iv. 在执行方法调用的时候会调用生成的字节码文件，然后直接找到 DynamicAdvisedInterceptor 类中的 `intercept()` 方法来开始执行

   v. 接着根据之前定义好的通知来生成拦截器链

   vi. 从拦截器链中依次获取每一个通知来执行，在执行过程中为了方便找到下一个通知，会有一个 CglibMethodInvocation 对象，找的时候是从 -1 位置依次查找并执行
...
9. **SpringBoot 如何实现的自动装配？**

   还是用的 Spring IOC 容器里面的扩展功能点，用 ConfigurationClassPostProcessor 来实现 ，在内部方法中会去调 `parse()` 来解析 `@Componet` `@PropertySource` `ComponentScan` `@Import` `@Bean` 等注解

10. **Spring 事务的隔离级别有哪些？**

    事务的隔离级别就是数据库的隔离级别，参考 MySQL 那里，如果数据库的和 Spring 代码中的隔离级别不同那么会以 Spring 的配置为主

11. **Spring 是如何加载配置文件**

    i. 靠的是 `loadBeanDefinitions()` ，里面还有有很多同名的方法，是因为我们在配置文件中有很多标签，例如 `<context:>`

    ii. `<aop:>` `<tx:>` 等，每个标签包含了不同的元素属性，需要独特的处理器来对配置文件进行处理

    iii. 进行读取的时候最基本的得先用 IO 流读取出字符串

    iv. 然后需要把这特殊的字符串通过 dom4j 等方式转换为 Document 对象（这个时候就有固定格式了，可以对父子标签进行处理）

    v. 遍历每一个子元素来进行解析操作

    vi. 当遍历 `<bean>` 元素的子元素时，每个子元素的解析方式是不一样的，就需要根据不同的元素找到对应的处理器 handler

    vii. 对不同的元素进行解析操作，解析完之后一定会把解析后的值放到 BeanDefinitions 对象里面
...
12. **Spring 事务是如何回滚的？**
    ...
    i. 先做准备工作，解析各个方法上事务相关的属性，根据具体的属性来判断是否开始新事务

    ii. 当需要开启的时候，获取数据库连接，关闭自动提交功能，开起事务

    iii. 执行具体的 sql 逻辑操作

    iv. 在操作过程中，如果执行失败了，那么会通过 completeTransactionAfterThrowing 看来完成事务的回滚操作，回滚的具体逻辑是通过 doRollBack 方法来实现的，实现的时候也是要先获取连接对象，通过连接对象来回滚

    v. 如果执行过程中，没有任何意外情况的发生，那么通过 commitTransactionAfterReturning 来完成事务的提交操作，提交的具体逻辑是通过 doCommit 方法来实现的，实现的时候也是要获取连接，通过连接对象来提交

    vi. 当事务执行完毕之后需要清除相关的事务信息 cleanupTransactionlnfo

    > 如果想要聊的更加细致的话，需要知道 TransactionInfo,TransactionStatus，

13. **你肯定知道 Spring，那说说 AOP 的全部通知顺序 SpringBoot1 或 SpringBoot2 对 AOP 的执行顺序影响？ | SpringBoot1 和 SpringBoot2 有什么区别**

    SpringBoot1 到 SpringBoot2 相当于是 Spring4 升级到了 Spring5

    - 升级引入了 WebFlux
    - Spring 通知的顺序发生了改变，@After 变成了最后执行

14. **@Transactional 注解在什么情况下会失效**
...
18. **Dao 接口的工作原理是什么？Dao 接口里的方法，参数不同时，方法能重载吗？**
...
![RabbitMQ](http://cdn.liumulin.top/interview/1*HoAG-7IhLaXShPJG-g9kvA.png)
...
1. **为什么使用消息队列啊**
   ...
   - 解耦
     ...
     ![MQ_01_MQ在解耦场景中的使用](http://cdn.liumulin.top/interview/MQ_01_MQ%E5%9C%A8%E8%A7%A3%E8%80%A6%E5%9C%BA%E6%99%AF%E4%B8%AD%E7%9A%84%E4%BD%BF%E7%94%A8.jpg)
   ...
   - 削峰
     ...
     ![MQ_03_使用MQ来进行消峰的场景](http://cdn.liumulin.top/interview/MQ_03_%E4%BD%BF%E7%94%A8MQ%E6%9D%A5%E8%BF%9B%E8%A1%8C%E6%B6%88%E5%B3%B0%E7%9A%84%E5%9C%BA%E6%99%AF.jpg)
   ...
   - 异步
     ...
     ![MQ_02_使用MQ进行异步化之后的接口性能优化](http://cdn.liumulin.top/interview/MQ_02_%E4%BD%BF%E7%94%A8MQ%E8%BF%9B%E8%A1%8C%E5%BC%82%E6%AD%A5%E5%8C%96%E4%B9%8B%E5%90%8E%E7%9A%84%E6%8E%A5%E5%8F%A3%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96.jpg)
...
4. **如何保证消息队列的高可用**
   ...
   - 镜像队列
     ...
     ![MQ_04_RabbitMQ的镜像队列模式](http://cdn.liumulin.top/interview/MQ_04_RabbitMQ%E7%9A%84%E9%95%9C%E5%83%8F%E9%98%9F%E5%88%97%E6%A8%A1%E5%BC%8F.jpg)
...
5. **如何保证消息不被重复消费啊（如何保证消息消费时的幂等性）**
   ...
   **场景：**
   ...
   - 在生产者开启 confirm 模式后，每次写的消息都会分配一个唯一 ID。那么在准备消费的时候我们先根据这个 ID 去 Redis 里查一下有没有消费过。如果没有消费过就进行业务处理然后将这个 ID 写到 Redis；如果发现 Redis 里面已经有这个 ID ，代表消费过了就不做任何处理避免重复消费
...
6. **如何保证消息的可靠性传输（如何处理消息丢失的问题）**
   ...
   - **RabbitMQ 弄丢消息**
     ...
     问题：RabbitMQ 为了高吞吐量，是在接收到消息之后暂存在内存中的。但如果消费者还没来得及消费就挂了的话，会导致内存中数据丢失，所以需要用持久化机制来避免数据丢失

     方案：持久化机制

     i. 创建队列时设置为持久化，这样就可以保证 RabbitMQ 持久化 queue 的元数据刷到磁盘上，但是它不会持久化 queue 里的消息

     ii. 发送消息的时候将消息的 deliveryMode 设置为 2，就是将消息设置为持久化的。只有这两个配置要同时配置上，才能保证 RabbitMQ 挂了重启之后也能从磁盘上恢复 queue 里面的消息
   ...
   - **消费者弄丢消息**
     ...
     RabbitMQ 提供了消费者应答（ack）机制，默认情况下是自动应答，只要消息推送到消费者就会autoAck 接着从队列中删除消息。但是消费端接收到消息执行业务代码报错了没执行成功或是宕机了。消费端就返回 ack 消息给 RabbitMQ，然后把这条消息删除了，这消息就丢失了

     方案（手动应答）：关闭自动应答改为启用手动应答，然后在消费端成功处理完业务之后再调用 API 手动 ack 确认，最终才让 RabbitMQ 从队列删除这条消息
...
8. **有几百万消息持续积压几小时，如何解决消息队列的延时以及过期失效问题？消息队列满了以后该怎么处理？说说怎么解决**
   ...
   [【IT老齐046】项目案例分享，宜信如何利用RabbitMQ队列解决消息积压问题？_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Wq4y1f79c/?vd_source=4844de7cb051be29fbaf4555af0bbd8b)
...
10. **RabbitMQ 有哪几种工作模式**
    ...
    > RabbitMQ 常用的交换器类型有 direct（直连）、 topic、fanout（广播）、 headers（headers 键值对全匹配）这四种
...
3. <span id='RedLock'>**知道分布式锁吗？有哪些实现方案？**</span>
   ...
   i. 基于 Redis：利用 setnx（set not exist）命令，就是只有在某个 key 不存在情况才能 set 成功返回 1，如果 set 失败的话返回一个 0，只要这个值存在就是谁先往 Redis 里面设置值谁就获得这个锁，后续 setnx 失败的就没有这个资格，就要等其它锁释放以后才能进行访问，这样就达到了多个进程并发去 set 同一个 key，只有一个进程能 set 成功
...
4. **谈谈你对 Redis 分布式锁的理解。Redis 做分布式锁有哪些需要注意的问题？（Redis 删 Key 的时候有哪些问题？）**
   ...
   问题2. <span id='redisLockLose'>==Redis 异步复制造成锁丢失==</span>（CAP + 集群对比 Zookeeper）现在 Redis 是单机版，但一般情况下我们会配一主两从或三主三从，它什么都好就是容易在集群环境下出现 CAP 的一些小故障，主机 master 加锁后会里面同步给 slave 从机，而 Redis 的特性是 AP（高可用 + 分区容错），没有保证数据一致性，导致的问题就是 master 刚刚获得了锁就返回了 OK，但是 master 还没同步回 slave 的时候 master 就宕机了，假如我们有高可用的哨兵或自动重启的无人值守，master 降级为 slave，其他 slave 上位成新的 master。由于 master 宕机的时候还没有同步锁给 slave，slave 上位后发现没有锁。这就 Redis 异步复制就会造成锁丢失
   ...
   >
   > AP（高可用 + 分区容错）：保证了高可用就牺牲了数据一致性
   >
   > CP（数据一致性 + 分区容错）：Zookeeper 主节点挂了以后有选举算法，它加锁以后不会着急回复，它会让从节点数据都和主节点一致了之后才会回复我这边加锁成功了
   >
...
5. **Redis 缓存过期淘汰策略**
   ...
   - **Redis 内存（缓存）淘汰策略**
    ...
    - **Redis 的 LRU 了解过吗？可否手写一个 LRU 算法**
      ...
      - 是什么：LRU 是 Least Recently Used 的缩写，即 `最近最少使用` 缓存机制。是一种常用的页面置换算法，选择最近最久未使用的数据给淘汰掉
      ...
      ![LRU cache 的 get 操作流程动图演示](http://cdn.liumulin.top/interview/LRU%20cache%20%E7%9A%84%20get%20%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B%E5%8A%A8%E5%9B%BE%E6%BC%94%E7%A4%BA.png)

         - 手写 LRU

           ![LRU principle](http://cdn.liumulin.top/interview/LRU%20principle.png)
...
7. **Redis线程模型**
   ...
   Redis 是单线程的，客户端发送请求到 socket，通过 IO 复用模型把消息压入队列，文件事件处理器会调用之前关联好的事件处理器来处理这个事件并返回 OK
...
11. **Redis的主从复制原理能介绍一下么？（主从架构的核心原理）**

    ![Redis主从复制的原理](https://cdn.jsdelivr.net/gh/liuilin/interview@latest/interview_note/img/redis主从复制的原理.png)

    当启动一个 slave node 的时候，它会发送一个 PSYNC 命令给 master node，如果这是 slave node 重新连接 master node，那么它只会复制部分缺少的数据给 slave；否则如果是 slave node 第一次连接 master node，那么会触发一次 full resynchronization 全量复制。开始 full resynchronization 的时候，master 会启动一个后台线程，开始生成一份 RDB 快照文件，同时还会将从客户端收到的所有写命令缓存在内存中。RDB 文件生成完毕之后，master 会将这个 RDB 发送给 slave，slave 会先写入本地磁盘，然后再从本地磁盘加载到内存中。然后 master 会将内存中缓存的写命令发送给 slave，slave 也会同步这些数据。

12. **Redis 的哨兵原理能介绍一下么？**
...
13. **RDB 和 AOF 两种持久化机制的介绍（Redis 挂了怎么办）**

    RDB (Redis DataBase)：对 Redis 中的数据执行周期性的持久化

    AOF (Append Only File)：对每条写入命令作为日志，以 append-only 的模式写入一个日志文件中（现代操作系统中，写文件不是直接写磁盘，会先写 os cache ，然后到一定时间再从 os cache 到 disk file。每隔 1 秒调用一次操作系统 fsync 操作强制将 os cache 中的数据刷入磁盘文件中）

    > 通过 RDB 或 AOF，都可以将 Redis 内存中的数据给持久化到磁盘上面来，然后可以将这些数据备份到别的地方去，比如说阿里云，云服务
    >
    > 如果 Redis 挂了，服务器上的内存和磁盘上的数据都丢了，可以从云服务上拷贝回来之前的数据，放到指定的目录中，然后重新启动 Redis，Redis 就会自动根据持久化数据文件中的数据，去恢复内存中的数据，继续对外提供服务

    优缺点及选择：综合使用 AOF 和 RDB 两种持久化机制，用 AOF 来保证数据不丢失，作为数据恢复的第一选择；用 RDB 来做不同程度的冷备，在 AOF 文件都丢失或损坏不可用的时候，还可以使用 RDB 来进行快速从云端恢复数据

14. **Redis 集群模式的工作原理能说一下么？**
...
15. **如何处理 Redis 雪崩？**
    ...
    what 缓存雪崩：某一时刻大规模的缓存失效，比如你的缓存服务宕机了就会有大量的请求进来直接打到 DB 上。结果 DB 抗不住就挂掉，像这样的连锁反应就是缓存雪崩

    方案：

    - 事前：采用 Redis cluster 来保证 Redis 高可用，避免全盘崩溃

    - 事中：在系统内先使用本地缓存（ecache、Guava） + sentinel 限流 & 降级，避免 MySQL 被打死
...
16. **如何处理 Redis 缓存穿透？**
    ...
    - 那么我们就可以为这些 key 对应的 value 值设置为 null 丢到缓存里面去并设置过期时间。那后面再出现查询这个 key 请求的时候呢就会走缓存直接返回 null
    - 在缓存之前在加一层布隆过滤器 (BloomFilter)，它就是一个 bitmap 嘛，在查询的时候先去 BloomFilter 去查询 key 是否存在，如果不存在就直接返回，存在再去查缓存

    > 如何选择
    >
    > 针对 key 异常多、请求重复率比较低的数据，我们就没有必要进行缓存，使用第二种方案直接过滤掉；而对于空数据的 key 有限的，重复率比较高的，我们则可以采用第一种方式进行缓存

17. **如何解决缓存击穿**

    what 缓存击穿：在平常高并发的系统中，大量的请求同时查询一个key时，此时这个key又刚好失效了，就会导致大量的请求都打到数据库上
...
19. **高并发场景下的缓存 + 数据库双写不一致问题分析与解决方案设计**
    ...
    更新数据的时候，根据数据的唯一标识，将操作路由之后，发送到一个 jvm 内部的队列中读取数据的时候，如果发现数据不在缓存中，那么将重新读取数据 + 更新缓存的操作，根据唯一标识路由之后，也发送同一个 JVM 内部的队列中一个队列对应一个工作线程每个工作线程串行拿到对应的操作，然后一条一条的执行这样的话，一个数据变更的操作，先执行，删除缓存，然后再去更新数据库，但是还没完成更新此时如果一个读请求过来，读到了空的缓存，那么可以先将缓存更新的请求发送到队列中，此时会在队列中积压，然后同步等待缓存更新完成。读操作呢可以做个排重去优化

    > 按 1：99 的比例计算读和写的请求，每秒 5 万的读 QPS，可能只有 500 次更新操作如果一秒有 500 的写 QPS，那么要测算好，可能写操作影响的数据有 500 条，这 500 条数据在缓存中失效后，可能导致多少读请求，发送读请求到库存服务来，要求更新缓存一般来说，1：1，1：2，1：3，每秒钟有 1000 个读请求，会 hang 在库存服务上，每个读请求最多 hang 多少时间，200ms 就会返回在同一时间最多 hang 住的可能也就是单机 200 个读请求，同时 hang 住单机 hang200 个读请求，还是 ok 的 1：20，每秒更新 500 条数据，这 500 秒数据对应的读请求，会有 20 * 500 = 1 万 1 万个读请求全部 hang 在库存服务上，就死定了

20. **Redis并发竞争问题以及解决方案？分布式锁？**

    分布式锁用的是 zookeeper（redis），确保同一时间，只能有一个系统实例在操作某个 key，别人都不允许读和写每次要写之前，先判断一下当前这个 value 的时间戳是否比缓存里的 value 的时间戳要更新，如果更新，那么可以写如果更旧，就不能用旧的数据覆盖新的数据
...
![万家美慧架构图](http://cdn.liumulin.top/interview/%E4%B8%87%E5%AE%B6%E7%BE%8E%E6%85%A7%E6%9E%B6%E6%9E%84%E5%9B%BE.jpg)
...
8. **接口定义规范（接口是怎么定义的？采用什么数据格式？如何实现？）**
...
1. **三家公司的离职原因**
   ...
   i. 我们那是分公司，入职后负责的是项目维护，这个项目只有我和贵阳那边的一个同事。后来那个同事离职了要求我过去那边办公，那边不好招人，然后我就拒绝走了

   ii. 因为我交了女朋友，搬家离公司有点远了就换了一家

   iii. 公司里从上海移动回来的架构师被排挤走了，那个新来的领导要把我也搞走。本来我就也想走了，想的是架构师走了，团队里里面的凝聚力也散了，我就先准备一下先撤，一个团队走不走的长远，架构师还是很关键的。

2. **CAP 定理**

3. **项目里面遇到过什么难点问题，怎么解决的?**

   多线程导致的问题采用的是 fixThreadPool 线程池，有 20 多亿的队列位置。

   应该自己来 new 并设置核心线程和队列数等等
...
9. **关于项目上线的问题？ 比如：并发多少，集群环境，搭建了多少台服务器**

   集群环境是否负责： 生产环境我作为开发并没有涉及，主要负责的事开发环境和测试环境。

10. **当面试官问你还有什么问题问我时，回答模式？**
...
11. **SpringBoot 注解：**
    ...
    装配一个 Bean 时，会用到 @Configuration、@Bean 这两个注解。这种装配方法虽然简单粗暴，但是多了都用 @Bean 方式注入会很痛苦，那就可以使用 SpringBoot 的方式装配 Bean，使用的就是

    @Component（标明哪个类被扫描，在类里面使用 @Value 指定值）和 @ComponentScan，当然 @SpringBootApplication 里面已经包含了 @ComponentScan。
    ...
    @SpringBootApplication：包含了 @ComponentScan、@Configuration 和 
...
24. **用 JQuery 写一段伪代码，实现页面端通过 Ajax 调用服务端请求**
...
[你在选offer时，如何避开烂offer？](https://www.zhihu.com/question/51833228/answer/1572213063)

[你在选offer时，如何避开烂offer？ - 木鸟杂记的回答 - 知乎 ](https://www.zhihu.com/question/51833228/answer/2272717122)

|                       | 万网科技                                               | 一蟹科技                                         |
| --------------------- | ------------------------------------------------------ | ------------------------------------------------ |
| 规模（人）            | 100-500（开发人员50人）                                | 120（开发人员30人）                              |
| 五险一金              | 最低                                                   | 不是最低，说在慢慢根据工资调整（有点模糊）       |
| 距离                  | 8km                                                    | 5km                                              |
| 项目                  | 核心是车贷（所有入职人员都需要了解），可能接手其他项目 | 医疗方向 SaaS，2B面向医药生成厂家                |
| 架构                  | 好像只说了是微服务                                     | Dubbo+zk                                         |
| 成立时间              | 2007                                                   | 2015                                             |
| 注册/实缴资金(天眼查) | 500w                                                   | 200w                                             |
| 基本工资/绩效         | 好像是90%（绩效大多数人正常拿满）                      | 70%（30%绩效，说的基本上都会拿满）不知道是否真实 |
| 调薪晋升              |                                                        | 一年两次调薪                                     |
| 加班                  | 好像说年底项目忙，要加班（包括偶尔周六加班）           | 除了项目上线外不加班                             |
...
> 消息队列，保证可靠性，提交了必须得消费掉，人工处理也得处理掉
>
