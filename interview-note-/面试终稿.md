...
1. ~~**为什么要使用多线程（你了解的多线程）**~~
   ...
   **多任务**
   ...
   而 CPU 执行代码都是一条一条顺序执行的，但即使是单核 CPU，也可以同时运行多个任务。因为操作系统执行多任务实际上就是让 CPU 对多个任务轮流交替执行。例如，让浏览器执行 0.001 秒，让 IDEA 执行 0.001 秒，在人看来，CPU 就是在同时执行多个任务。 即使是多核 CPU，因为通常任务的数量远远多于 CPU 的核数，所以任务也是交替执行的。
   ...
   **使用场景**
   ...
   >
   > 生产中不要中 `new Thread()` 方式，这样数据量大的话会导致系统中线程越来越多，CPU 资源占用越来越大，上下文切换也越来越快，性能就急剧下降。可以使用线程池来固定 n（比如 10 个，一般每个核跑一个线程就可以了，8 核就设置 8 个）线程

2. **Java 实现多线程有哪几种方式？JDK 1.0 就已经有了 Runnable 接口了，为什么还会有 Callable 接口呢？**
...
5. **请谈谈你对 volatile 的理解（volatile 关键字的作用 | 请你谈谈 JMM）**
   ...
   那讲解 volatile 的特性之前呢需要先了解到 <font color='red'>JMM（Java 内存模型 Java Memory Model）</font>

   JMM：JMM（Java 内存模型 Java Memory Model, 简称 JMM）本身是一种抽象的概念，主要用它来解决并发过程中的可见性、有序性和原子性问题，具体实现方式有很多，像刚才提到的 volatile 就是其中之一

   JVM 进程中实际底层去运行的是线程，每个线程创建时 JVM 都会为其创建一个工作内存（也就是本地内存，有些地方成为栈空间）。而 JMM 中规定所有变量都必须存储在主内存中，主内存是共享内存区域且所有线程都可访问，但如果<font color='red'>线程要对变量进行操作（读取或赋值等）就必须在工作内存中进行</font>，首先要将变量从主内存拷贝到自己的工作内存，然后对变量进行操作，操作完之后再将变量写回主内存。
   ...
   > iii. 加锁解锁是同一把锁
   >
   > 由于工作内存是线程的私有区域，因此线程之间无法访问对方的工作内存；由于不能直接操作主内存中的变量，线程之间的通讯（传值）必须通过主内存来完成，工作内存存的是共享变量副本

     ![JMM 内存模型](http://rhqfw1q3k.hn-bkt.clouddn.com/interview/JMM%20%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B.jpg)

   **volatile 是 JVM 提供的比 synchronized 轻量级的同步机制，它有三大特性：保证内存的可见性、禁止指令重排、但不保证原子性。**
...
6. **讲一讲 Atomiclnteger，为什么要用 CAS 而不是 synchronized？（CAS知道吗？如何实现的？CAS有什么缺陷，该如何解决）**
...
8. **谈谈你对 synchronized 关键字的理解（synchronized关键字的用法，优缺点 | synchronized 和 Lock 的区别）**
...
10. **公平锁、非公平锁、可重入锁、递归锁、自旋锁，谈谈你对它们的理解？请手写一个自旋锁。**
...
11. **CountDownLatch、CyclicBarrier、Semaphore 使用过吗？**
...
16. **Dao 接口的工作原理是什么？Dao 接口里的方法，参数不同时，方法能重载吗？**
...
18. **Mybatis 都有哪些 Executor 执行器？它们之间的区别是什么？**
