...
> 临阵磨枪：线程池、AQS、分布式事务 Seata 原理、Spring IOC、Spring AOP、Bean 流程、Spring 循环依赖、MySQL（聚簇索引、非聚簇索引）、Redis 原理、反射，jvm 调优（线上调优参数），NIO、IO 多路复用（Redis）
>
...
>
> [聊聊 Linux 五种 IO 模型 - 简书](https://www.jianshu.com/p/486b0965c296)
...
2. **谈谈 Spring IOC 的理解，原理和实现**
   ...
   > IOC 在 Spring 代表的是容器，取东西需要 K，V。创建 Bean 需要定义信息也就是 BeanDefinition，可以用 xml或注解方式，但是开发人员使用不同的方式定义。那就需要一个通用的、抽象的进行读取，用到的是 BeanDefinitionReader
   >
   > xxPostProcessor 要立马反应到是做增强功能的
   >
   > Environment 里的 `system.getEnv` `system.getProperties` 就是程序启动时加载进了容器，我们可以直接获取

   - what：

     - 控制反转 (Inversion of Control)

       理论思想：原来的对象是由使用者来进行控制，有了 Spring 容器之后，可以把整个对象交给Spring 来帮我们进行管理

       Dl（Dependency Injection）：依赖注入，把对应的属性的值注入到具体的对象中，`@Autowired`，`populateBean()` 完成属性值的注入

     - 容器

       存储对象，使用 map 结构来存储，在 Spring 中一般用三级缓存 singletonObjects 来存放完整的 bean 对象，整个 bean 的生命周期，从创建到使用到销毁的过程全部都是由容器来管理（bean 的生命周期）

     > 我想在 A 类中调用 B 类的方法，那么 A 类中就需要得到 B 类对象之后才能调用。这个过程统一交给 Spring 管理就叫 IOC

   - why：降低耦合度

   - how（原理 | 源码 refresh()）：

     1. 首先是容器就需要创建容器工厂，容器上层有个 BeanFactory 的根接口，没有提供子类实现，我们实际调用中最常用的就是 DefaultListableBeanFactory，然后会向 bean 工厂中设置一些参数（像 BeanPostProcessor、Aware 接口的子类）等等属性

     2. 加载解析 bean 对象，准备要创建 bean 对象的定义对象 beanDefinition，（xml 或者注解的解析过程）

     3. beanFactoryPostProcessor 的处理，这里是扩展点，Spring 为什么有这么好的生态发展的那么好就是因为 Spring 提供了很多扩展点，比如常见的 PlaceHolderConfigureSupport（占位符处理），ConfigurationClassPostProcessor（BeanPostProcessor 的注册功能，方便后续对 bean 对象完成具体的扩展功能

     4. 通过反射的方式讲 BeanDefinition 对象实例化成具体的 bean 对象

     5. bean 对象的初始化过程（填充属性，调用 aware 子类的方法，调用 BeanPostProcessor 前置处理方法，调用 init-mehtod 方法，调用 BeanPostProcessor 的后置处理方法）

     6. 生成完整的 bean 对象，通过 getBean 方法可以直接获取生成完整的 bean 对象，通过 getBean 方法可以直接获取

     7. 销毁过程

        

     1. 首先是容器就需要创建容器工厂，容器上层有个 BeanFactory 的根接口是 DefaultListableBeanFactory 工厂

     2. 加载 Bean 定义信息，`loadBeanDefinitions(beanFactory)`

     3. `prepareBeanFactory(beanFactory)` 进行设置属性值的前戏操作

     4. 调用 `invokeBeanFactoryPostProcessors(beanFactory)` 进行执行扩展操作
     ...
     > `initMessageSouce()` 初始化 is8n 国际化等相关资源。不是很重要可以不看

   面试官，这是我对 IOC 的整体理解，包含了一些详细的处理过程，您看一下有什么问题，可以指点我一下（允许你把整个流程说完）
   老师，我没看过源码怎么办？
   具体的细节我记不太清了，但是spring中的bean都是通过反射的方式生成的，同时其中包含了很多的扩展点，比如最常用的对
   BeanFactory的扩展，对bean的扩展，我们在公司对这方面的使用是比较多的，除此之外，ioc中最核心的也就是填充具体bean的属性，
   和生命周期（背一下）。
   ...
   要把 xml 配置文件（或者 properties、yaml以及注解等方式）进行加载、解析变成一个 BeanDefinition 对象（bean 的定义信息）。但是这么多种配置文件转换的方式肯定不一样，加入后面还想出一直 JSON 文件配置来定义 bean 对象，这时候就需要考虑在他们前面加个抽象类 BeanDefinitionReader，给出一个固定的接口，让不同的子类去实现即可（相当于定义了一个规则）。

   我们通过 Spring 容器呢最想获取到里面的对象，所以接着该实例化对象，可以通过 new、反射以及工厂来实例化。如果这样就结束了的话岂不是太简单了，那 Spring 这么伟大的框架存在的意义是啥？Spring 框架它是 SpringBoot、SpringCloud 等框架的基石，所以它需要有最重要的一个特点——扩展。如果太简单就没办法做更多的事，用户就没法自定义做一下东西了。当我们需要访问一个容器的时候，Spring 提供了一个访问的根接口 BeanFactory，在 bean 实例化前有一个或多个扩展点，可以实现 BeanFactoryPostProcessor 来做一些扩展，比如 PlaceholderConfigurerSupport 类对占位符做替换处理。

   接着就该实例化对象了，可以反射 `Constructor ctor = Clazz.getDeclareConstructor();` + `Object obj = ctor.newInstance()` 来实例化对象

   > 获取 Class 的三种方式：`Class.forName(xxx)` `类.Class` `对象.getClass()`

   然后开始初始化对象，也就是 bean 的生命周期

   实例化好了之后代表对象已经有了，然后对对象进行扩展，主要在 before、after 这里完成 BeanPostProcessor 接口的扩展点来实现 AOP
   ...
   >
   > 动态代理选择方式的源码：BeanPostProcessor、AbstractAutoProxyCreator.postProcessAfterInitialization()、wrapIfNecessary()、createProxy()、getProxy()、getProxy()

   

   **使用 IOC 的推导过程（了解即可）**

   xml 解析或注解解析、工厂模式、反射（通过得到类的字节码文件，然后可以操控内存中所有内容）

   原始直接 new 的方法耦合度太高，假如 dao 类的路径变了，那 service 里面也得跟着变，可以用工厂来进行解耦，让 UserService 不依赖于 UserDAO

   ```java
   // 这种方式还是有耦合，工厂容器依赖了 DAO
   class UserFactory {
   ...
   改进：需要用 xml 解析或注解解析 + 反射来把耦合度降到最低
   ...
   class UserFactory {
       public static UserDAO getUserDAO() {
           // xml 解析，拿到全类名 com.at.guigu.UserDao
           String className = xxx工具类解析操作;
   ...
   优势：之前那种假如 dao 类的路径变了，那 service 里面也得跟着变。现在是只用改 xml 配置文件里的全类名，其他地方完全不需要修改，耦合度进一步降低

   IOC 底层就是对象工厂，Spring 提供 IOC 容器两种方式
   ...
   - BeanFactory：加载配置文件的时候不会创建对象，在获取对象使用时才创建对象。（像懒汉模式）
   - ApplicationContext：加载配置文件的时就会创建对象，（像饿汉模式）

   > 一般用第二种，在耗时、耗资源的操作在程序启动时就初始化

   **DI（了解即可）**：依赖注入，就是注入对象的属性值，需要在创建对象的基础之上去完成
...
4. **Spring 是如何解决循环依赖问题的**
   ...
   what：A 依赖 B，B 依赖 A（A 对象里面有个 b 属性，B 对象里面有个 a 属性，形成了一个闭环）

   此时，如果仔细琢磨的话，会发现 A 对象是存在的，只不过此时的 A 对象不是一个完整的状态，只完成了实例化但是未完成初始化，如果在程序调用过程中，拥有了某个对象的引用，能否在后期给他完成赋值操作，可以优先把非完整状态的对象优先赋值，等待后续操作来完成赋值，相当于提前暴露了某个不完整对象的引用，所以解决问题的核心在于实例化和初始化分开操作，这也是解决循环依赖问题的关键

   当所有的对象都完成实例化和初始化操作之后，还要把完整对象放到容器中，此时在容器中存在对象的几个状态，完成实例化但未完成初始化，完整状态，因为都在容器中，所以要使用不同的 map 结构来进行存储，此时就有了一级缓存和二级缓存，如果一级缓存中有，那么二级缓存中就不会存在同名的对象，因为他们的查找顺序是 1,2,3 级这样的方式来查找的
...
7. **Spring AOP 的底层实现原理？**
   ...
   what：面向切面编程 (Aspect-Oriented Programming)，它只是 IOC 的一个扩展功能点 （一个BeanPostProcessor），我们通过它能够在不修改源码的情况下对功能进行增强。一般应用于系统中的日志、事务等（与业务无关的）场景上

   分：（助记：b-代-J-字节码-通知-拦，bean代表JJ叫字节码通知拦截器链）
...
10. **你肯定知道 Spring，那说说 AOP 的全部通知顺序 SpringBoot1 或 SpringBoot2 对 AOP 的执行顺序影响？ | SpringBoot1 和 SpringBoot2 有什么区别**
...
13. **RDB和AOF两种持久化机制的介绍（Redis挂了怎么办）**
