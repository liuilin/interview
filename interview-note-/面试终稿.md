...
1. **面向对象的特征**
   ...
   >
   > HashMap 里面的 Node 节点就是实现类 Map.Entry，就可以用里面的属性以及重写方法
   >
   > ```java
...
3. **接口和抽象类有什么区别？**
   ...
   接口是对行为的抽象，是抽象方法的集合，利用接口可以达到 API 定义与实现分离的目的。接口不能够被实例化，也不能包含任何非常量成员，所以所有字段都是隐含着 public static final 的含义；同时方法呢要么是抽象方法，要么是静态方法。

   > 如 JDK 库中的 List 接口。分离 - 字段 - 方法

   抽象类是不能实例化的类，抽象类大多用于抽取相关 Java 类的共用方法实现或者是共同成员变量，然后通过继承的方式达到代码复用的目的。在标准库中，比如 Collection 框架，很多通用部分就被抽取成为抽象类，例如 Java.util.AbstractList

   **总结：**

   > - 抽象类也是类，也就说明类只能单继承。但接口可以多实现
   >
   > - 如果实现了一个接口，那么必须要实现接口的所有方法，因为接口中只能有抽象方法的。
   >
   >   而抽象类中可以是抽象方法，也可以不是抽象方法从而避免必须在子类中重复实现。而只有抽象方法需要强制实现
   >
...
5. **一个 ArrayList 在循环过程中删除，会不会出问题，为什么**
   ...
   有问题。遍历的时候应该使用 Iterator 或 倒序遍历 List
...
6. **Java 容器有哪些？哪些是同步容器？哪些是并发容器？（Java 集合之间的区别 | 介绍一下集合框架）**
   ...
   - Collection
     ...
     - ArrayList 和 LinkedList（List 实现类）
       ...
       iv. 如果调用的是无参构造，那么第一次添加元素会扩容 elementData 为 10，需要再次扩容就为 elementData 的 1.5 倍；而使用的是指定 capacity 容量的构造器，那么 elementData 容量就为 capacity 的值，扩容的时候大小为传入值的 1.5 倍
       
       
       
       LinkedList 是个双向链表 ，特点是查改慢（要进行依次遍历）、增删快（因为只用移动指针）
       ...
       i. LinkedList 底层维护了一个双向链表，里面有两个属性值 first 和 last 分别指向了首节点和尾结点
       
       ii. 每个节点（Node 对象）又维护了 prev、next、item 这三个属性，通过 prev 指向前一个节点，next 指向后一个节点，最终就实现了双向链表。所以 LinkedList 的添加和删除元素效率特别高，因为它不需要数组扩容啊那些操作
       
       > 所以要是查改多就用 ArrayList，增删多就用 LinkedList，一般来说项目里面 80-90% 都是查询，大部分情况下是使用 ArrayList，但是 ArrayList 是线程不安全的，可以使用 `Vector` 或 `Collections.synchronizedList(new ArrayList<>());` 或者 `CopyOnWriteArrayList` 来保证线程安全
     ...
     - HashSet 实现了 Set 接口
       ...
       3. 找到存储数据表 table，看这个索引位置是否已经存放的有元素。如果没有，直接加入；如果有，调用 equals 比较，如果相同，就放弃添加，如果不相同，则添加到最后
       4. 在 Java8 中，如果一条链表的元素个数到达 TREEIFY THRESHOLD（默认是 8），并且 table 的大小 >=MIN TREEIFY CAPACITY（默认 64）就会进行树化（红黑树）
     
       扩容原理：
     
       HashSet、HashMap 底层是，第一次添加时， table 数组扩容到 16，临界值（threshold）是 16 * 加载因子（LoadFactor）是 0.75=12 如果 table 数组使用到了临界值 12，就会扩容到 16 * 2=32 新的临界值就是 32 * 0.75=24，依次类推
     
       如果 table 数组使用到了临界值 12 就会扩容到 16 * 2=32，新的临界值就是 32 * 0.75=24，依次类推
     
       在 Java8 中，如果一条链表的元素个数到达 TREEIFY THRESHOLD（默认是 8）并且 table 的大小 >=MIN TREEIFY CAPACITY（默认 64）就会进行树化（红黑树），否则仍然采用数组扩容机制
   ...
   - Map
     ...
     - Map 中的 key 不允许重复，原因和 HashSet 一样，前面分析过源码，Map 中 value 的可以重复
...
7. **我们知道 ArrayList 是线程不安全的，请编写一个不安全的案例并给出解决方案（方案：CopyOnWriteArrayList）**
   ...
   比如我在名单上写名字的时候，有人上来就争抢我的笔把我写的名字搞乱了。那我希望我写操作时能够加锁来保证原子性，一次只有一个人操作；但同时我又希望大家读的时候不加锁来提高并发性，让所有人都能同时看到这份名单，而不是一个人一个人的看

   **原理：**CopyonWrite 写时复制容器。它往一个容器添加元素的时（add 写操作），是先将当前容器 Object[] 进行拷贝，复制出一个新的容器 Object[] newElements 里，然后往新的容器里添加元素，添加完元素之后，再将原容器的引用指向新的容器（ setArray 步骤）；这样做的好处是可以对 CopyonWrite 容器进行并发的读而不需要加锁（写的时候 add 方法加入了 lock 锁）
...
8. **HashMap 底层数据结构？实现原理？源码？**
   ...
   [【源码解析】用Java手写（Spring、Tomcat、Hashmap、Mybatis、SpringBoot...）框架源码，挑战年薪70W_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1S64y1o7S6?p=40)

   [【韩顺平讲Java】Java集合专题 -ArrayList HashMap HashSet List Map TreeMap TreeSet等_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1YA411T76k?p=28&spm_id_from=pageDriver)
   ...
   >
   > ==但是取模运算效率太低了，所以 HashMap 里面用的为位运算（之前试了下 xx 的数据量，位运算效率能达到取模运算的 16 倍之多）==

   - 1.7 是数组 + 链表（ArrayList + LinkedList）

     数组查询快，根据下标就可以直接获取到对应的数据。但是它增删慢，如果是加在末尾当然不影响，但如果是往中间插入元素时就需要往后移动元素

     链表增删快，不用移动元素，只需要移动指针。因为它要一个一个的去遍历

     HaspMap 就是综合了它两的各自的优点

     > 每一条数组链叫做 bucket，源码有

     数组可以放基本数据类型，也可以放引用数据类型。所以数组里至少应该放置的是个对象，`key` `value` 肯定得有，还需要 `hash` 通过 `key` 计算一个哈希值并存储。下面要是有相同 `hash` 的数据时，需要用链表来连接起来，那就需要一个 `next` 指针

     ```java
     // 在源码中验证（大胆的进行猜测，然后去源码中验证）
   ...
   - 1.8 是数组 + 链表 + 红黑树
     ...
     **底层原理**
     ...
     - 当我们调用空参构造方法来创建对象时，会将加载因子（loadFactor）初始化为 0.75
     - 当执行 put 方法添加 key、value 时，通过 key 的哈希值得到在 table 数组上的索引。然后判断该索引处是否有元素，如果为空就创建节点。如果该索引处有元素了，会继续判断该元素的 key 和准备加入的 key 他们对应的 hash 值 和 equals 是否相等，如果相等新值就会去覆盖它；如果不相等需要判断是树结构还是链表结构，然后做出相应处理。如果添加时发现容量不够了就需要扩容
     - 当我们第 1 次添加元素的时候，需要扩容 table 容量为 16，临界值（threshold）为 16*0.75=12，以后再扩容，则需要扩容 table 容量为原来的 2 倍，临界值也为原来的 2 倍即 24 依次类推
     - 在 Java8 中，如果一条链表的元素个数超过 TREEIFY_THRESHOLD 树化的临界值 8，并且 table 的大小 >=MIN_ TREEIFY_CAPACITY 最小树化容量 64，接着就会进行树化成红黑树
     ...
     **链表用的好好的，为什么要从链表转为红黑树呢？**
     ...
     > 而它不是每次都是按照树来搜索，里面有个 `树化` 和 `反树化` 的过程（链表和红黑树之间的转换）
     
     **在 new HashMap() 的时候，需要指定数组的长度，数组长度为什么是 2 的 n 次幂？**
     ...
     树化的两个前提：i. 单个桶长度（链表长度）达到 8 ii. 整体节点个数超过 64
     
     扰动函数：本来普通进行 hash 的时候直接调用 `hashCode()` 方法就好了，而 HashMap 里面是用哈希值 ^ 与上哈希值右移 16 位，这样做是为了让散列更加均匀，让值能够均匀的落在每一个数组上，不然会浪费很多空间
   ...
   static final int hash(Object key) {
     ...
     **HaspMap 是怎样扩容的？**
     ...
     resize() 方法一开始是在进行初始化，容量为 16。而当链表中的 K,V 键值对个数超过了阈值就会扩容

     数组一旦创建时不能修改的，只能创建新的数组，然后把老数组的值移动到新数组？

     > `& 与运算（位运算）` 是要比 `% 取模运算` 效率高很多，测试有 16 倍
     >
     > & 与运算：同 1 为 1，有 0 则为 0。
     >
     > 必须要 2 的 n 次幂，才能让与运算代替取模运算
     >
     > ++size 放的是 map 中 K,V 键值对的个数；就是我们每次加入的一个节点 Node(k,v,h,next)，不管是数组上的还是链表上的节点都算。只要 size > threshold 就会进行 resize() 扩容

     JDK 8 之后链表都是尾插法，头插法有循环的问题
     ...
     ==如果从第一次开始一直往同一个链表加元素到达 8 个，也会扩容为 32，再加一个元素会扩到 64，再加一个元素就会把当前链表转为红黑树==

9. **Hash冲突问题（为什么会有hash冲突算法）**

   为什么会有 hash 冲突算法 (n-1)&key.hashCode () 呢：我们想要让它均匀的落在每个节点上，避免链表过长

   hash 的操作：首先是根据我们的 key 进行一个哈希。然后右移 16 位取到它的高 16 位，再与他的本身也就是低 16 位进行异或 ^ 操作。取异或的原因是因为想要 0、1 均衡，尽量让每一位都参与到运算中来，避免出现一种极端情况全是 0 或 1，那就没意思了

   数组的操作：数组长度必须是 2 的 n 次幂，因为二的 n 次幂是一个偶数，偶数减 1 是一个奇数。是个基数，在进行计算的时候，二进制最低位一定是一个 1，这时和 hash 值进行一个与 & 运算时，计算出来 key 落在数组的索引值是奇数还是偶数是依赖的数组长度二进制最高位对应的 hash 值是 0 还是 1。如果数组长度是一个奇数，减 1 得到一个偶数后，无论你的 hash 值是多少，计算出来索引的永远只能是一个偶数，这就意味了奇数位就用不了
...
11. **HashMap、HashTable 和 TreeMap 有什么不同？为什么要 ConcurrentHashMap？（HashMap 在高并发下如果没有处理线程安全会有怎样的安全隐患，具体表现是什么）**
    ...
    - Hashtable 是早期 Java 类库提供的一个哈希表实现，不支持 null 键和值，本身是同步的加了锁（读写都加了锁），导致效率比较低，所以已经很少被推荐使用
    
    - HashMap 是应用更加广泛的哈希表实现，行为上大致上与 HashTable 致，主要区别在于 HashMap 不是同步的，支持 null 键和值。通常情况下， HashMap 进行 put 或者 get 操作，可以达到常数时间的性能（就像电脑里的快捷访问文件夹，它作为 Key，点击之后立马跳转到对应的 Value 目录，就不用再一层层去翻了。）
